...
//
//   "No order selected"        — a detail pane opened without a record
...
// period, add the first variant — and it is the module's own hue that says which
// part of the product is inviting you. silica's EmptyState paints a 55%-faded
// base-content glyph and takes no color, so the tone is applied here, once, for
// the same reason it is applied inside the other two: the caller passes its
// glyph, the component decides what the state looks like.
...
export function PaneEmpty({
  ...
  return (
    <div className="flex h-full min-h-72 flex-col items-center justify-center gap-1 px-6 py-10">
      <EmptyState
        icon={icon ? <span className="text-module">{icon}</span> : undefined}
        title={title}
