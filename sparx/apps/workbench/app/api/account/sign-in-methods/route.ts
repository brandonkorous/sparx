...
// `useSession()`. The surface used to take it off the session user, which is a
// fact the cookie only carries until the flag next flips -- and on the Piggles
// console, where /api/auth/get-session is not served at all, it answered
// undefined forever and the card's badge read a flat "Off" on accounts that had
// it switched on. A lookup that never answered is not a measurement, and it must
// not render as one.
//
