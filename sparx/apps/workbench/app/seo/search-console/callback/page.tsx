...
// Google Search Console OAuth landing.
//
// Google redirects the small consent popup here with ?code=&state= (or ?error=
// if the owner cancels). This page runs ONLY in that popup: it hands the values
// back to the workbench window that opened it via postMessage, then closes
// itself. The exchange for a stored connection happens in the Search Console
// pane, on the workbench's own token — nothing sensitive is handled here, and
// the code/state are same-origin data passed straight through.
//
  ...
  return (
    <div className="bg-base-200 grid min-h-screen place-items-center p-8">
      <div className="card bg-base-100 flex max-w-sm flex-col gap-2 p-6 text-center">
        ...
        <p className="text-sm">
          {closed
            ? 'This window can be closed. Head back to the workbench to see your connection.'
