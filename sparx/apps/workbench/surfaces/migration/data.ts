...
// `@wizeworks/migration`, with no network call at all, so a file that cannot be read
 ...
 * email because a mailing list is. Color here is wayfinding into the module the
...
/** Status → the semantic color it should wear. State is its own color axis. */
