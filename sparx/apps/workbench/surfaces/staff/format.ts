...
// says so in words before it says so in a color.
//
// COLOR IS THE DESIGN, not decoration on it (DESIGN.md RULE #4). Every state
// function here returns a tone, because on all six of these surfaces the color
 ...
 * rate priced which shift. `dayKey` in @wizeworks/staff makes the same argument for
 * the server half.
 *
...
/**
 * A qualification's state, in words and color.
 *
