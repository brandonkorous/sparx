...
/** A single pre-send check (mirrors `@wizeworks/email`'s `EmailCheck` — duplicated because
...
// against their own colors and type rather than a neutral preset. This only tints
...
/** The tenant brand as `/v1/brand` returns it — the color/type columns the theme
 ...
 *  semantic colors the inbox gets (not the site page theme, which can diverge), and
 ...
 * site's override laid on top (docs/49). Only the token-bearing fields (color,
