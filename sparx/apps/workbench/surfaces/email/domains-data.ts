...
//
// ── The endpoints (wizeworks/services/api-rest/.../email/domains.ts) ─────────────────
  ...
  switch (domain.state) {
    case 'verified':
      return {
        label: 'Verified',
        tone: 'success',
        detail: 'This address is proven, so sparx can send your email from it.',
      };
    case 'verifying':
      return {
        label: 'Checking',
...
export function recordPurpose(record: DnsRecord): string {
  ...
  if (value.includes('v=spf1')) {
    return 'Lists sparx as allowed to send email on your behalf.';
  }
  if (record.recordType.toUpperCase() === 'CNAME') {
    return 'Lets sparx handle your links and track opens and clicks.';
  }
