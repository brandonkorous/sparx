      ...
      onSuccess: (updated: SendingDomain) => {
        if (updated.state === 'verified') {
          toast.add({
            ...
            description: 'sparx can send your email from this address now.',
            type: 'success',
          });
        } else {
  ...
  const onDelete = async () => {
    const ok = await confirm({
      ...
      description: isDefault
        ? `This is the address this site sends from by default. Removing it means email falls back to a generic sparx address until you set another. Email you have already sent is unaffected. This cannot be undone.`
