...
// ── WHY THIS EXISTS ────────────────────────────────────────────────────────
//
// `conversionFunnel()` has been computed, exposed over REST and offered as an MCP
// tool for as long as it has existed, and NOTHING drew it. A tenant could see
// their own conversion rate only by asking their AI (docs/152 A1). This is that
// gap closed.
//
// ── EVERY RATE HERE IS `number | null` ─────────────────────────────────────
//
// And null renders as words, never as 0%. That distinction is the whole reason
// the report was rewritten in A1: a business selling through wholesale, the
// counter or the phone has no web sessions at all, and "0% conversion" tells
// them their storefront is failing when they do not have one.
...
/** Each step as a share of the first, so the narrowing is visible rather than
 *  arithmetic. No bar when nothing reached the top — a full-width bar over zero
 *  visits would claim 100% of nothing. */
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <ol className="flex flex-col">
        {rows.map((step, index) => {
          const width = shareOfTop(step.count, top);
          return (
            <li key={step.label} className="flex flex-col">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                {width === null ? null : (
                  <Progress
                    color={index === rows.length - 1 ? 'module' : 'info'}
                    value={width}
