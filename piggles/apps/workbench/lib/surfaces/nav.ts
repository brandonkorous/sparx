...
export function buildNav(): NavModule[] {
  ...
  for (const [module, surfaces] of byModule) {
    ...
    for (const surface of sorted) {
      // The BRAND's word for this group, falling through to the platform's. Done
      // at grouping time rather than at render, so two platform sections a brand
      // has deliberately collapsed onto one name ("Setup" and "Setting up") land
      // in ONE bucket instead of two identically-titled ones.
      const key = surface.section
        ? (productSectionTitle(surface.section) ?? surface.section)
        : null;
