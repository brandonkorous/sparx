...
export function useViewer() {
  return useQuery({
    ...
    queryFn: async () => {
      const state = await getTokenState();
      return { userId: state.userId, role: state.role, name: state.userName };
    },
    staleTime: Infinity,
  });
}
...
/**
 * The operator's FIRST name, for chrome that speaks to them.
 *
 * First name only, and that is the whole reason this is a function rather than
 * a field: a greeting that says "Good morning, Marta Oyelaran" is addressing a
 * record, and one that says "Good morning, Dr Oyelaran-Whitfield" runs off the
 * end of the line. Splitting on the first space is crude and right — it is what
 * a person would do reading a name aloud.
 *
 * Empty string when the account has no name, which callers render as a greeting
 * with no name in it rather than an empty gap after a comma.
 */
