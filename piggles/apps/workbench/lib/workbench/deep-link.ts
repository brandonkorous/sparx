...
export function readDeepLink(address?: string): DeepLink | null {
  ...
  if (path !== '/') {
    // This console's brand, so `/home` opens ITS Home rather than the other
    // product's. Every other address in the table resolves the same either way.
    const matched = matchPath(path, url.searchParams, 'piggles');
    if (matched) {
