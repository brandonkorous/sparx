...
export interface WorkbenchTokenResponse {
  ...
  role: string;
  /**
   * The operator's own name, for chrome that addresses them rather than
   * describing them — Home says "Good morning, Marta".
   *
   * Same caveat as the two above: a DISPLAY value, never a control. It rides
   * this route because panes render in the browser and a detached window renders
   * in a different document entirely, so a prop threaded down from the server
   * component cannot reach either.
   *
   * Empty string when the account has no name set — a real state, and the
   * caller's job to fall back rather than to render "undefined".
   */
  userName: string;
}
