 ...
 *  is an account setting; the `staff` app is the workforce (hours, rota, licences)
 *  and is a different set of people asking a different question. */
...
export function TeamMemberSurface({ ctx }: { ctx: SurfaceContext }) {
  ...
  const { member, ready, isError, missing, refetch, isFetching, updatedAt } =
  ...
  // All three non-ready states fill the same shell and the same card the loaded
  // pane does, so nothing about the chrome moves as the teammate arrives. The
  // failure takes the empty-state shape rather than an Alert: an Alert is a
  // banner over content that is still there, and here there is none.
  if (isError) {
    return (
  ...
  // after someone was removed, so it is `missing` rather than a failure — the
  // wrapper tones it warning and suppresses a retry that could not work.
  if (missing || !member) {
    return (
      <div className={PANE_SHELL}>
        <Card className="min-h-0 flex-1 overflow-y-auto">
  ...
  return (
    <div className={PANE_SHELL}>
      {/* The bar always renders — it carries this pane's refresh and its copy
          link, which a read-only viewer needs as much as anyone. Only SAVE is
      ...
      <PaneToolbar
        ...
        refresh={
          <RefreshButton isFetching={isFetching} updatedAt={updatedAt} onRefresh={refetch} />
        }
      />
...
function MemberActivity({ userId, name }: { userId: string; name: string }) {
  const { items, ready, failed, retry } = useActivity({ actorId: userId, limit: 25 });
  ...
  return (
    ...
    >
      {failed && items.length === 0 ? (
        // Split out of the empty branch: a failed read left `items` empty, and
        // "they have made no changes" is a claim about a person's work — the
        // last thing to guess at when reviewing their access.
        <PaneLoadError
