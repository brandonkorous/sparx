  ...
  return (
    <div className={PANE_SHELL}>
      <PaneToolbar
        ...
        views={{
          target: '/platform/settings/integrations',
          params: { q: search.trim(), status, category: category ?? '' },
          onApply: (next) => {
            setSearch(next.q ?? '');
            setStatus((next.status as StatusFilter | undefined) ?? 'all');
            // An empty string is "every category", which this surface spells
            // `undefined` — a saved view stores nothing for a cleared filter.
            setCategory(next.category === '' ? undefined : next.category);
          },
        }}
        refresh={
          <RefreshButton
