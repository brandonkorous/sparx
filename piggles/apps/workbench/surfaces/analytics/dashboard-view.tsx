...
export function DashboardViewSurface({ ctx }: { ctx: SurfaceContext }) {
  ...
  // Both early states fill the same card the dashboard's tiles do, so the pane
  // does not go from a bare recessed surface to a grid of lifted cards.
        ...
        <Card className="min-h-0 flex-1 overflow-y-auto">
          <PaneEmpty
            ...
            description={productCopy(
              ...
              'It comes with an app this business has not switched on yet.'
            )}
          />
        </Card>
      </div>
        ...
        <div className="@container mx-auto flex w-full max-w-[1600px] flex-col gap-4 p-4">
          {!config ? (
            ...
            <>
              ...
              {query.isError ? (
                /* Carded like the tile grid beside it — the figures are what is
                   missing, and the region that holds them keeps its frame. */
                <Card>
                  <PaneLoadError
                    module={module}
                    icon={<Icon glyph={faGauge} className="size-6" aria-hidden />}
