...
// which SERIES colors we want — so we resolve the ambient module hue (and a
 ...
 *  an element inside the themed subtree, plus the resolved colors (empty until
...
function useTokenColors(names: readonly string[]): {
    ...
    useEffect(() => {
        ...
        // concrete colors change even though the token names don't.
...
/** The ambient module hue as a CONCRETE color (ECharts on canvas can't read a
...
// The dot color for each legend row, in the SAME order as DONUT_TOKENS, so the
 ...
 * type, every time. Slice colors and legend dots walk the module-led palette in
 * lockstep.
 */
