...
export function BinLabelsSurface({ ctx }: { ctx: SurfaceContext }) {
  ...
  const { data, isLoading, isFetching, dataUpdatedAt, refetch } = useBins({
  ...
  return (
    <div className={PANE_SHELL}>
      ...
      {/* ONE card around the whole conditional — loading, empty and the sheet all
          fill the same content region, so the pane never jumps. The printed copy
          is portaled to <body> (see components/print-sheet.tsx), so the card is a
          screen-only frame and nothing reaches the paper. */}
      <Card className="min-h-0 flex-1 overflow-y-auto">
        {isLoading ? (
          <PaneWaiting module={MODULE} label="Loading shelves…" />
        ) : bins.length === 0 ? (
          <PaneEmpty
            module={MODULE}
