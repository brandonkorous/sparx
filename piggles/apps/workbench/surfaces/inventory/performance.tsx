          ...
          {report.unattributedUnits > 0 ? (
            <>
              {productCopyWith(
                ...
                `${plural(report.unattributedUnits, 'unit', 'units')} sold through a channel whose order lines Piggles never saw, so their sales are missing from the figures.`,
                { units: plural(report.unattributedUnits, 'unit', 'units') }
              )}
            </>
          ) : null}
        </GapNote>
