  ...
  return (
    <section className="card bg-base-100 flex flex-col gap-3 p-4">
      ...
      {data.sources.length === 0 ? (
        <Text className="text-sm">
          {productCopy(
            'inventory.provenance.internalOnly',
            'This number is kept in Piggles — nothing outside is feeding it.'
          )}
        </Text>
      ) : (
        <ul className="flex flex-col gap-2">
...
export function ProvenanceSurface({ ctx }: { ctx: SurfaceContext }) {
  ...
  // The item names the tab; the qualifier keeps it from reading as the item's
  // own pane, since this one explains a number rather than showing the thing.
  const itemName = data ? (data.productTitle ?? data.variantSku ?? null) : null;
  useEffect(() => {
    if (itemName) ctx.setTitle(`${itemName} · where this came from`);
  }, [ctx, itemName]);
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        <div className="mx-auto flex w-full max-w-3xl flex-col gap-3">
          {query.isLoading ? (
            ...
            <>
              {/* The tab names the item; WHERE is the other half of what
                  addresses this number, and only lives here. */}
