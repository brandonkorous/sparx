...
export function StockGridSurface(_props: { ctx: SurfaceContext }) {
  ...
  /** Something is narrowing the grid, so an empty result is the query's doing
   *  rather than an empty catalog. */
  const narrowed = Boolean(search || warehouseId || lowOnly);
  ...
  return (
    <div className={PANE_SHELL}>
      ...
      {/* ONE card around the whole conditional — loading, failure, empty and the
          grid all fill the same content region, so the pane never jumps.
          The waiting and failed branches are new: before them an unfinished load
          rendered "No stock to show", which is a measurement nobody took. */}
      <Card className="min-h-0 flex-1 overflow-auto">
        {grid.isError ? (
          <PaneLoadError
            ...
            description="This is a problem reaching the server. Nothing you have counted has changed — it just could not be read just now."
            onRetry={() => {
              void grid.refetch();
            }}
          />
          ...
          <ListEmptyState
            ...
            noResults={{
              ...
              description: 'Try a shorter search, or clear the location filter.',
            }}
            firstRun={{
