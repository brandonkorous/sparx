 ...
 * nested `ModuleScope` — color follows FUNCTIONALITY, so a commerce line reads
 * commerce-orange inside this platform-hued pane rather than everything wearing
 * one color (docs/23 Color-Follows-Functionality).
 */
...
export function ActivityFeed({ hasJobs }: { hasJobs: boolean }) {
  ...
  const { items, ready, busy, hasMore, failed, retry } = useActivity({
    limit: pageSize,
    ...(before ? { before } : {}),
  });
  ...
  // Split out of the empty branch below: a failed read left `items` empty, so a
  // connection blip was telling somebody nothing had happened in their business.
  // Only when the feed is actually empty — a failed poll over rows still on
  // screen leaves them be.
  ...
  if (ready && items.length === 0 && cursors.length === 0) {
    return (
      <FormSection title="What's happened">
        {/* No Card: the FormSection is one. <PaneEmpty> for the brand's picture
            — this section is the pane's main content region. */}
        <PaneEmpty
          module={MODULE}
          icon={<Icon glyph={faWavePulse} className="size-6" aria-hidden />}
