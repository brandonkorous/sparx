...
export function FeedbackThreadSurface({ ctx }: { ctx: SurfaceContext }) {
  ...
  const { data, error, isPending, isError, isFetching, dataUpdatedAt, refetch } = useFeedbackThread(
    id || null
  );
  ...
  // A 404 means the message is gone; anything else means the server could not be
  ...
  return (
    <div className={PANE_SHELL}>
      ...
      <PaneToolbar
        ...
        // A reply from the team lands on the server with nothing on screen to
        // say so — this pane sits open while you wait for one.
        refresh={
          <RefreshButton
