...
export function ProductOptionsTab({ product }: { ctx: SurfaceContext; product: Product }) {
  ...
  // button invites someone to rebuild a lattice on top of nothing. Carded like
  // every other state of this tab — the ready one is a stack of FormSections, so
  // an uncarded failure sat on the bare tab surface and the body jumped as it
  // recovered. The empty-state shape rather than an Alert: an Alert is a banner
  // over content that is still there, and here there is none.
...
function NoChoicesYet({ onAdd, hadChoices }: { onAdd: () => void; hadChoices: boolean }) {
  return (
    // Carded, because the branch beside it is a stack of FormSections — each
    // already a card. <PaneEmpty> rather than a bare EmptyState so the brand's
    // picture appears here as it does in every other state of this tab.
    <Card>
      <PaneEmpty
        module={MODULE}
        icon={<Icon glyph={faShapes} className="size-6" aria-hidden />}
