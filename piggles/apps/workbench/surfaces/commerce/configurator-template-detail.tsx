  ...
  if (draft === null) {
    return (
      <div className={PANE_SHELL}>
        <PaneToolbar label="Build actions" status={<span className="ml-auto" />} />
        <div className="min-h-0 flex-1 overflow-y-auto">
          <div className={COLUMN}>
            <Text>
              A build turns a product into something a shopper makes to order. Start by choosing the
              product it is for.
            </Text>
...
function Editor({
  ...
  /** Only the manage flow has a query behind it; a brand-new build has none. */
  isFetching?: boolean;
  updatedAt?: number;
  onRefresh?: () => void;
}) {
