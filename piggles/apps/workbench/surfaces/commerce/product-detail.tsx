  ...
  return tabSaveProvider(
    <div className={PANE_SHELL}>
      <PaneToolbar
        ...
        // Save sitting behind an extra tap is the whole bug this rule exists for.
        // It commits the ACTIVE tab only; the dots on the tab strip say which
        // other tabs still have work pending. A tab with nothing to save never
        // registers, and Save is simply absent there rather than present-but-dead,
        // which would be a worse lie. Enforced by scripts/check-toolbar-primary.mjs.
        primary={
          tabSave.active ? (
            <Button
              size="sm"
        ...
        // Everything the surface OFFERS rather than exists for. As values, so
        // each keeps its name in the popover instead of collapsing to a glyph —
        // and so Share stops being bespoke JSX that renders unlike its
        // neighbours there.
        actions={[
          ...(onSale && productUrl
            ? [
        ...
        refresh={
          <RefreshButton
            ...
            updatedAt={product ? dataUpdatedAt : undefined}
            onRefresh={() => {
              void refetch();
            }}
          />
