        ...
        switch (action.kind) {
            ...
            case 'show_only_choices':
                return `${labelOf(action.optionKey)} only offers ${action.choiceKeys
                    ...
                    .join(', ')}`;
            case 'price_adjust':
                return `the price changes by ${deltaLabel(action.deltaCents, currency) ?? 'nothing'}${action.label ? ` (${action.label})` : ''
                    }`;
            case 'add_addon':
                ...
                >
                    <Text>
                        ...
                        {bundle.pricingMode === 'fixed'
                            ? `The set costs a flat ${bundle.fixedPriceCents === null ? 'price you have set' : ''
