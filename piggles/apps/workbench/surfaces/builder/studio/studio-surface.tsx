...
export function StudioSurface({ ctx }: { ctx: SurfaceContext }) {
  ...
  if (site.isError && site.data === undefined) {
    return (
      // An <Alert> is a banner ABOUT content that is still on screen. Here there
      // is no content — the first load failed — so it was a stray error box in
      // an empty pane. The editor has no toolbar to preserve either (its chrome
      // is built from the document that did not arrive), so the card IS the
      // frame: the message sits where the canvas would have been, at the depth
      // content sits at. See components/pane-load-error.tsx.
  ...
  if (loading) {
    return <PaneWaiting label="Loading the editor…" />;
  }
