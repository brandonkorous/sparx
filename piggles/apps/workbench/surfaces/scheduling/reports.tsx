...
// the live `--color-*` tokens and re-inits on a light/dark flip. Series colours
...
/* ── Token colours off the DOM (canvas can't read a CSS var) ─────────────── */
...
// meaning. The order is fixed so a slice keeps its colour even when another
// outcome has zero and drops out — semantic colour, not position, is the point.
const OUTCOME_DEFS = [
...
function OutcomeRing({ totals }: { totals: SchedulingReportTotals }) {
  ...
  // All-or-nothing: a partial colour array would misalign hue to slice. Until
...
export function SchedulingReportsSurface({ ctx: _ctx }: { ctx: SurfaceContext }) {
  ...
  const body = () => {
    ...
    if (report.isPending || !report.data) {
      return <InlineWaiting label="Working out your figures…" />;
    }
