...
//
// The whole page wears the app's GROUP hue via `data-group`, so the colour a
// visitor sees while reading about Stock is the colour Stock wears in the rail
// after they sign up.
...
export default async function AppPage({ params }: { params: Promise<{ app: string }> }) {
  ...
  return (
    <div data-group={app.group}>
      <PageHero heading={copy.heading} lede={copy.lede}>
        <a
              ...
              {copy.worksWith.map((otherId) => {
                ...
                return (
                  <Card key={otherId} data-group={other.group}>
                    <CardBody>
                      <h3 className="text-module text-lg font-bold">
              ...
              <p className="text-base">
                ...
                {siblings.map((s, i) => (
                  <span key={s.id}>
                    ...
                    <Link href={`/apps/${s.id}`} className="text-module font-semibold">
                      {s.label}
                    </Link>
                  </span>
                ))}
