...
export default function BrandPage() {
  return (
    <main>
      {/* On the same hero as every other page. This one is a reference surface
          rather than a marketing one, so it takes no call to action — but a
          reference page that opens differently from the pages it is the
          reference FOR is demonstrating the opposite of its own argument. */}
      <PageHero
        heading={`The ${PRODUCT.name} theme, rendered.`}
        lede="Every value here was chosen by measurement rather than by eye; this page is where those numbers meet a browser. Both themes render at once, because a token that passes in one mode and fails in the other is exactly what a toggle hides."
        figure={<BrandFigure />}
      />
