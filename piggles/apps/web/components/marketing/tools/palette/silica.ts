 ...
 * block re-colours every button, badge, input, tab, alert and focus ring at once
 ...
 *     the moment the role colour changes. An ink the visitor OVERRODE is a
...
/** Swatches no role claimed. They are still part of the palette, and a colour
 *  silica has not been told about renders as nothing at all — `btn-colour-5`
 *  compiles, lints, and paints an unstyled button. */
function extras(palette: Palette): [string, string][] {
  return palette
    .slice(ROLE_ORDER.length)
    .map((s, i): [string, string] => [`colour-${ROLE_ORDER.length + i + 1}`, s.hex]);
}
 ...
 *  silica's to measure, and stays right when the role colour changes.
...
export function silicaTheme(
  ...
  return [
    ...
    // `<Badge color="warning">` comes out colourless.
