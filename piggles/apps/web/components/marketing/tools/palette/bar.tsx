 ...
 * `neutral` is the banned version of this, and `soft` tints a color that isn't
 * there.
 */
function Round({
    icon,
...
export function PaletteBar({
    ...
    return (
        <div className="rounded-section border-base-300 bg-base-100 flex flex-wrap items-center gap-3 border p-3 shadow-md">
            ...
            <Choice
                label="How the colors relate to each other"
                value={scheme}
                onChange={onScheme}
                options={(Object.keys(SCHEMES) as Scheme[]).map((k) => [k, SCHEMES[k].label])}
            />
            <Choice
                label="See the palette through a different kind of color vision"
                value={vision}
                onChange={onVision}
