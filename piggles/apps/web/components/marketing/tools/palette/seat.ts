...
/**
 * How well a color suits the slot it is being considered for.
 *
 * Lower is better. Position IS the role, so a shuffle that returns the right
 * five colors in the wrong order returns a broken theme — a dark green sitting
...
function cost(role: Role, oklch: { l: number; c: number } | null): number {
    ...
    switch (role) {
        ...
        case 'base-100':
            return 1 - oklch.l;
        // Chrome wants the least colorful thing, and a dark one — it is also where
        ...
        // The color roles want color, in descending order of how much.
        case 'primary':
            return 1 - oklch.c * 3;
        case 'accent':
...
/**
 * Put the colors in the slots that suit them.
 *
 * A LOCKED SWATCH KEEPS ITS SLOT, which is what makes a lock mean "this color,
 * in this job" rather than merely "this color, somewhere". Everything else is
