 ...
 * blue at L 50, so a set built by nudging HSL comes out with the warm colours
 * shouting and the cool ones sunk. OKLCH is perceptually uniform — asking for
 * L 0.62 gives five colours that genuinely read as equally bright, which is what
...
export const SCHEMES: Record<Scheme, { label: string; blurb: string }> = {
  balanced: {
    label: 'A working set',
    blurb: 'One colour to lead, something to read on, and a paper to sit it all on.',
  },
  complement: {
    label: 'Opposites',
  ...
  analogous: {
    label: 'Neighbours',
    blurb: 'Colours that already live together. Calm, and hard to get wrong.',
  },
 ...
 *  L 0.27 it stops reading as a colour and starts reading as ink. */
...
/**
 * The ladder steps still available once the locked colours have taken theirs.
 *
 ...
 * this did first — spends them on colours that are not going to be generated:
 ...
 * taken out of the ladder, so locking a brand colour and shuffling gives a set
...
function reroll(palette: Palette, scheme: Scheme, seed: number): Palette {
   ...
   * function of what is locked: with two colours pinned, two consecutive
   ...
   * colour left. With two colours locked there are only two rungs to fill, and
  ...
  return palette.map((current, i) => {
    ...
    // more colours. Giving them full chroma is what produced three browns and a
    // blue and called it a brand — real palettes carry one or two colours and a
...
/** One more colour, related to what is already there rather than random. */
