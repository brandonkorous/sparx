...
import { averageColor, drawSquare, loadImageFile, type LoadedImage } from './lib/canvas';
...
export function FaviconTool() {
  ...
  const [themeColor, setThemeColor] = useState('#FF6F86');
  ...
  const accept = async (files: File[]) => {
    ...
    try {
      ...
      // PNG has no idea their Apple icon needs an opaque color, and offering a
      // sensible one beats leaving the field white and hoping.
      if (loaded.hasTransparency) {
        setBackground(averageColor(drawSquare(loaded, { size: 64, fit: 'contain' })));
      }
    } catch (e) {
      setImage(null);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    setWorking(true);
    buildFaviconSet(image, { background, appName, themeColor })
      .then((built) => {
          ...
          {image ? (
            <Panel title="The details" description="Used for the phone and home-screen icons.">
              <ColorField
                ...
                hint="iPhones turn transparency black, so the home-screen icon has to be solid. This is the color behind your logo there."
                value={background}
