...
export async function buildDocumentPdf(input: DocumentInput): Promise<Blob> {
    ...
    const accent = pdfColor(input.accent);
    ...
    doc.text(words.title, RIGHT, y, { size: 26, font: 'bold', align: 'right', color: accent });
    y += image ? 52 : 34;

    doc.text(input.from.name || 'Your business', MARGIN, y, { size: 12, font: 'bold' });
    let fromY = y + 17;
    if (input.from.address) {
        fromY += doc.paragraph(input.from.address, MARGIN, fromY, 200, { size: 9.5, color: QUIET });
    }
    for (const line of [input.from.email, input.from.phone, input.from.taxId].filter(Boolean)) {
        doc.text(line, MARGIN, fromY, { size: 9.5, color: QUIET });
        fromY += 13;
    }
    ...
    for (const [label, value] of meta) {
        doc.text(label, RIGHT - 110, metaY, { size: 9, color: QUIET, align: 'right' });
    ...
    y += 16;
    if (input.to.address)
        y += doc.paragraph(input.to.address, MARGIN, y, 240, { size: 9.5, color: QUIET });
    if (input.to.email) {
        doc.text(input.to.email, MARGIN, y, { size: 9.5, color: QUIET });
        y += 14;
    }
    ...
    if (terms) {
        doc.text(words.termsLabel, MARGIN, y, { size: 9, font: 'bold', color: QUIET });
        y += 14;
        y += doc.paragraph(terms, MARGIN, y, RIGHT - MARGIN - 200, { size: 9.5, color: INK });
        y += 10;
    }
    if (input.notes) {
        doc.text('Notes', MARGIN, y, { size: 9, font: 'bold', color: QUIET });
        y += 14;
        doc.paragraph(input.notes, MARGIN, y, RIGHT - MARGIN - 200, { size: 9.5, color: INK });
    }
...
export function blankDocument(kind: DocumentKind): DocumentInput {
    ...
    return {
        ...
        // color picker — which is most people — and pink is also simply the wrong
