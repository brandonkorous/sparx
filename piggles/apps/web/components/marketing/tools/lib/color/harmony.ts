...
export const HARMONY_LABELS: Record<HarmonyKind, { label: string; blurb: string }> = {
  complementary: {
    label: 'Opposites',
    blurb:
      'Two colours from opposite sides of the wheel. High energy — good when one thing needs to stand out against everything else.',
  },
  analogous: {
    label: 'Neighbours',
    blurb:
      'Colours that sit beside each other. Calm and unmistakably related — good for backgrounds and large areas.',
  },
  triadic: {
    label: 'Three-way',
    blurb:
      'Three colours spread evenly around the wheel. Lively without clashing, and enough range for a whole brand.',
  },
  tetradic: {
    label: 'Four-way',
    blurb:
  ...
  random: {
    label: 'Surprise me',
    blurb:
      'Five colours picked at random, kept within a readable range. For when you have been staring at the same pink for an hour.',
  },
};
 ...
 *
 * The obvious version nudges each colour ±8 from the base, which produces five
 ...
 * the eye separates colours by lightness before it separates them by hue.
 */
const LIGHTNESS = [0, -22, 14, -11, 24];
const SATURATION = [0, -6, -14, 6, -20];

/** Five colours built from a base. Always five, so the UI never reflows when
...
export function harmony(base: Rgb, kind: HarmonyKind, seed = 0): Rgb[] {
  ...
  if (kind === 'random') {
    ...
    // re-rolls every locked colour on each render.
