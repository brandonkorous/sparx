...
export const SIGNATURE_LAYOUTS: { value: SignatureLayout; label: string; blurb: string }[] = [
  ...
  },
  {
    ...
    label: 'Photo beside',
    blurb:
      'Your photo or logo on the left with the details to the right, divided by a coloured rule.',
  },
  {
    value: 'minimal',
    label: 'Two lines',
...
export function buildSignature(input: SignatureInput): string {
  ...
  const link = (href: string, text: string, colour = accent) =>
    `<a href="${escapeHtml(href)}" style="color:${colour};text-decoration:none;">${escapeHtml(text)}</a>`;
