...
/**
 * Every control wears the page's app color.
 *
 ...
 * handed the form to default controls, so the color stopped at the band and
 ...
 * naming a color. Silica puts it on the border and the focus ring only — the
...
/**
 * Every control is a `FieldControl`, not a bare control inside a `Field`.
 *
 * ── THE LABELS POINTED AT NOTHING ───────────────────────────────────────────
 *
 * Base UI's Field mints an id, writes it into the label's `for`, and hands it to
 * Field.Control. A silica control dropped in as a plain CHILD never receives it,
 * so `<label for="base-ui-_R_7j…">` referenced an element that did not exist:
 * every field on all seventeen tools had no accessible name, and clicking a
 * label did nothing. Measured across the suite — five dead labels on the margin
 * calculator alone — and fixed here rather than at ~200 call sites, which is the
 * entire reason these wrappers exist.
 *
 * It renders identically either way, which is why it survived review for so
 * long. Styling goes on the control inside `render`; value, handlers and the
 * caller's `...rest` go on `FieldControl`, which merges them.
 */
...
export function TextField({
  ...
  return (
    <Field>
      ...
      <FieldControl
        render={<Input color={CONTROL_COLOR} size={CONTROL_SIZE} />}
        value={value}
...
export function AreaField({
  ...
  return (
    <Field>
      <FieldLabel className="text-base font-semibold">{label}</FieldLabel>
      {/* `rows` and the caller's rest go on the TEXTAREA, not on FieldControl:
          FieldControl is typed to HTMLInputElement, so a textarea-only attribute
          is a type error there. Base UI merges the render element's own props
          with the ones it computes, so both halves arrive. */}
      <FieldControl
        render={<Textarea color={CONTROL_COLOR} rows={rows} {...rest} />}
        value={value}
        onChange={(e) => onChange(e.target.value)}
      />
      {hint ? <FieldDescription className="text-base">{hint}</FieldDescription> : null}
...
export function SelectField<T extends string>({
 ...
 search or multi-select; none of these have any of that. */}
      <FieldControl
        render={<NativeSelect color={CONTROL_COLOR} size={CONTROL_SIZE} />}
        value={value}
...
/**
 * A color, as a swatch you can click and a hex box you can paste into.
 *
 * Both halves, always. The native color input is how somebody browses for a
 * color they have not chosen yet; the text box is how somebody who already has
 * a brand color on a piece of paper puts it in. Offering only the picker means
 * a business with a color has to hunt for it on a wheel.
 */
  ...
  return (
    <Field>
      ...
      <div className="flex items-center gap-3">
        {/* The color fills the control. A native swatch carries its own wrapper
        ...
        />
        <FieldControl
          render={<Input color={CONTROL_COLOR} size={CONTROL_SIZE} className="font-mono" />}
          value={value}
          onChange={(e) => onChange(e.target.value)}
          spellCheck={false}
...
/** A titled block. The heading wears the page's app color, so a long form reads
 ...
 *
 * This shipped with a 4px colored rule down its left edge, and that was wrong
 ...
 * everything else: scale, weight and color. A bold lead-in does the whole job,
