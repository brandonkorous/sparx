...
Found by asking a simple question about one screen: what colour is the text on
...
AA wants 4.5. These are 1.1 — the ink and the ground are the same colour to
...
Silica resolves a `soft`/`outline`/`ghost` variant by using the colour ITSELF as
...
It is not a colour that can be ink. And it was being asked to be ink 416 times.

## Where it lives

- [packages/brand/src/theme/palette.css](../../../packages/brand/src/theme/palette.css)
- [apps/workbench/app/globals.css](../../../apps/workbench/app/globals.css) — the registered colours, and the `[data-chrome]` island
...
**`neutral`** becomes what every other registered colour in that file already
...
colour instead. **This issue fixes how the token draws, not whether those 416 are
right to be grey.**
