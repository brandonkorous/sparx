...
This builds directly on the silica cutover ([118](118-builder-silicaui-html-migration.md)) and the customization rebuild ([98](98-builder-customization-rebuild.md)); the commerce composites live in `@wizeworks/silica-catalog`, not silicaui.

## Key facts established (source of truth)

- **Render path**: a stored silica page BODY → `renderSilicaBody` (`@wizeworks/silica-catalog`) → vendored `toHtml` → raw HTML; the FRAME is walked to React. The PDP renders from a **published stored template** (`getPublishedSilicaCollection`), so a composite code change alone does NOT change a live page — the stored tree must be re-authored too.
- **Data-need collection**: `collectSilicaSourceNeeds` (`@wizeworks/builder-schemas/silica-data-needs.ts`) walks a tree and reports which sources to fetch. Now reports **which** product sources are bound (`products: {catalog, featured, fresh, related, categories[]}`), not just a boolean.
- **Host resolution**: `buildSilicaHost` (`wizeworks/apps/site/lib/silica-data.ts`) fetches per-source and fills the resolver root. Bounded rails are capped (`FEATURED_LIMIT = 8`) and exclude the in-scope PDP product.
- **Sources**: `commerce.product` (whole catalog), `commerce.featured`, `commerce.new`, `commerce.related`, `commerce.category.<collectionHandle>`. Registered in `COMMERCE_SOURCES` (`@wizeworks/builder-schemas/binding.ts`) so the inspector's binding picker offers them.
- **Data realities**:
...
- **Editor palette**: `COMMERCE_CATALOG` (`@wizeworks/silica-catalog/catalog.ts`) is merged into silica's Insert palette; inserting a block **stamps its node tree** (forked, editable) — so card/PDP layout is already hand-editable.
...
- **B2 — Pinned functional component** — delivered by **silicaui 0.22's native `HostNode`** (the `BuilderHost` gap the audit surfaced was the upstream ask; 0.22 shipped it). The primitive, end to end:
  ...
  - **Single source of truth for the key vocabulary**: framework-free `@wizeworks/silica-catalog` owns the host-component **keys** + `HostComponentDef` metadata + the authoring composites (`host('commerce.checkout', …)`); the **dashboard** maps keys → canvas skeletons; the **site** maps keys → real functional components. No key is duplicated.
...
- `@wizeworks/silica-catalog/host-nodes.ts` — the React-free registry: `HOST_KEYS`, `HOST_COMPONENTS` (palette/inspector metadata), `hostCore(key)` (authors a `host()` node stamped `locked: "host"` so a **seeded** core is pinned, not just palette-inserted ones), and `functionalShell(key, {heading})` (the default editable shell). Tests in `host-nodes.test.ts`.
...
- Replacing silicaui's engine or generic blocks — commerce stays our domain layer in `@wizeworks/silica-catalog`.
