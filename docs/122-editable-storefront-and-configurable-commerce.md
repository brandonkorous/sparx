...
**Verticals landed so far (all gate-clean):** `commerce.cart` · `commerce.search` · `commerce.plp` · `commerce.collections` · `commerce.categories` (index) · `scheduling.services` (booking index). Each is a studio-editable seeded page + pinned live core. Starter commerce pages: Home, Shop, Cart, Search, Products, Collections, Categories, About, Contact; a scheduling tenant additionally seeds a **Book** page (`/book`) + a Book nav/footer link.
