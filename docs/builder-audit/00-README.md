...
> by the workbench (`apps/workbench/surfaces/builder/studio/`), the sparx `BuilderHost` that
...
- [02-silicaui-asks.md](02-silicaui-asks.md) — Wave 3: what went upstream and why. The register itself is [doc 139](../139-silicaui-builder-asks.md).
...
| Published reads, with the code starter as universal fallback                                 | **sparx**                                         | [apps/site/lib/silica.ts](../../apps/site/lib/silica.ts)                                     |
...
| 5   | **Responsive editing**             | **3** | No per-breakpoint authoring at all. The device toggle is a `max-width` on a div. ~~Half the catalog uses viewport variants the toggle physically cannot reflow.~~ _Vocabulary fixed ([slice 6](01-roadmap.md)); the authoring half is [doc 139 §1](../139-silicaui-builder-asks.md) and still open._ |
...
2. [apps/site/app/page.tsx:78](../../apps/site/app/page.tsx#L78) calls `getPublishedSilicaHome(site.slug)` with **no options**. `sp.sparxSitePreview` is not read until line 92, and the silica branch has already returned at line 84. `generateMetadata` does the same at line 41. ❌
...
5. [apps/site/app/layout.tsx:289](../../apps/site/app/layout.tsx#L289) calls `getPublishedSilicaFrame(site.slug)` — a signature with no token parameter — so draft header/footer edits never preview either. ❌

[apps/site/lib/silica.ts](../../apps/site/lib/silica.ts) builds an `Authorization: Preview` header that the server ignores, which is why this reads as working from the client side.
...
The engine itself ([packages/seo-audit/src/audit.ts](../../packages/seo-audit/src/audit.ts)) is good: 12
...
[02-silicaui-asks.md](02-silicaui-asks.md) and [doc 139 §1–2](../139-silicaui-builder-asks.md).
...
| Bound collections silently capped at 24, no pagination                             | `apps/site/lib/silica-data.ts` `COLLECTION_PAGE_SIZE` + the `console.warn` it emits                                                                                                                                                                                                                                                                                                                                                                                              |
...
| 23 `force-dynamic` storefront routes                                               | `apps/site/app/**`, despite a working `builder:<slug>` tag-purge pipeline                                                                                                                                                                                                                                                                                                                                                                                                        |
| No `srcset` / `sizes` generated for author-placed images                           | the attrs are allowlisted in `silicaui-html`; nothing emits them, and `packages/media` has no variant pipeline                                                                                                                                                                                                                                                                                                                                                                   |
...
**Noted, out of scope:** `apps/site/app/page.tsx` still carries four legacy render tiers beneath
