...
- **Dashboard color truth:** `packages/brand/src/theme.css` (`@sparx/brand`) — the silica token set: `--color-base-100/200/300` + `--color-base-content`, the semantic palette (`--color-primary/secondary/accent/neutral/info/success/warning/error/danger` + each `-content`), and the 18-module palette `--color-module-<name>`. The silicaui Tailwind plugin turns these into the `btn-*`/`bg-<color>`/`bg-soft` classes.
- **Dashboard non-color truth:** `packages/ui/src/tokens.css` — the `--text-*` scale (`--text-base: 1rem` "never below"), spacing, radii, shadow, motion, and the `--chart-*` palette. (Its old color block + the `.sx-c-*` role classes were deleted in the silicaui migration.)
