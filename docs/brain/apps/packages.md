...
**UI / design:** `@sparx/ui` (dashboard components) · `@sparx/site-ui` (storefront components, `--st-*`) · `@sparx/site-themes` (tenant theme presets + compiler) · `@sparx/web-chrome` (marketing header) · `@sparx/surface-compile` (per-tenant CSS compile). → [[design]], [[components]].
