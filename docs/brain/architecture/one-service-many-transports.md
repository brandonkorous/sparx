...
Every UI feature exists as an **API endpoint first**; the dashboard is one consumer among many. REST (`wizeworks/services/api-rest`), MCP (`wizeworks/services/api-mcp`), and GraphQL (`wizeworks/services/api-graphql`) are **separate first-class services** that dispatch into the **same module service layer** — "one service, many transports."
