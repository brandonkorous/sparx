...
status: active
sources:
  - wizeworks/packages/db/src/tenant-context.ts
  - wizeworks/packages/db/prisma/migrations/20260527000100_rls/migration.sql
...
- **Runtime:** wrap every tenant handler in `withTenant({ tenantId, userId }, fn)` (`wizeworks/packages/db/src/tenant-context.ts`) — it opens a `$transaction`, runs `SET LOCAL app.tenant_id = '<uuid>'`, and pins the query to that connection. Tenant id is request-scoped via `AsyncLocalStorage` (`tenant-store.ts`).
