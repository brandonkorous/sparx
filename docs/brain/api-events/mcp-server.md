...
status: active
sources:
  - services/api-mcp/src/server.ts
  - services/api-mcp/src/tool-registry.ts
---

`services/api-mcp` — a **first-class service**, not a plugin. `ALL_MCP_TOOLS` aggregates each module's read/write tool arrays (**~150 tools**: commerce ~32, crm ~34, sitebuilder 22, builder 18, automation 10, email 7, scheduling 7, inventory 6, search 5, domains 5). Tool source lives per package in `packages/<mod>/src/mcp/`.
