...
sources:
  ...
  - wizeworks/packages/email/src/providers/index.ts
  - wizeworks/packages/email/src/templates/
---

Outbound email defaults to **publishing `email.send` to Pub/Sub**. `email-worker` consumes, validates against a Zod discriminated union of template ids (or a pre-rendered `raw` broadcast), resolves per-site brand, calls `renderTemplate()` (`@wizeworks/email`, React Email → html + auto plaintext), then `getEmailProvider().send()`.
