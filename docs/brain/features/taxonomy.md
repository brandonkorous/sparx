...
sources:
  ...
  - packages/ui/src/providers/module-provider.tsx
---

Every capability is exactly one of three. The dividing line is **mechanical**, not a judgment call:

- **Module** — a key in `ModuleSlug` (`packages/modules/src/index.ts`). 12 of them. Each has a manifest, a catalog entry, a **Stripe price**, and a `settings.modules.<slug>.enabled` flag. Paid, toggleable, spectrum-hued. → [[modules]].
...
A **hue** in `module-provider.tsx` does **not** make something a module — that file deliberately carries hues for `partner`, `finance`, `seo`, `automations`, `storefront`, `platform`, and flags each in-file as "owns a hue, NOT a module."
