...
sources:
  ...
  - wizeworks/packages/modules/src/starters.ts
---
...
- **UI:** `(onboarding)/onboarding/` + `story/` + `welcome/`. **API:** `v1/public/signup`, `v1/industry-starters`, `v1/sample-data`; starters/presets in `wizeworks/packages/modules/src/starters.ts` + `presets.ts`. Writes tenant provisioning across domains (no dedicated table).
