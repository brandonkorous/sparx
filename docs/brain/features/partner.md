...
sources:
  ...
  - wizeworks/packages/db/prisma/schema/83-partners.prisma
---
...
- **Gating (two layers):** tenant opt-in (`partners` row) via `partner/_lib/access.ts` (`canOperatePartner`, `getPartnerAccess`); member role `PARTNER_OPS = {owner, admin, partner}` enforced in `wizeworks/services/api-rest/src/routes/v1/partner/` via `requireAnyRole`.
- **PRD:** docs/114 (Part B) + docs/78. **UI:** `(dashboard)/partner/` + `settings/partner/`. **API:** `v1/partner/*`, `v1/public/partners`, `internal/partners`; `wizeworks/packages/partner-schemas/`; no MCP. **Data:** `83-partners.prisma` (+ `84-bootcamps`).
