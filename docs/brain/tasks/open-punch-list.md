...
- **Migrate the drifted settings lists to `SelectionList`** — `settings/sites`, `settings/domains`, `settings/ai-integrations` hand-roll bespoke `Card`-row lists (no view toggle, no `defaultListView`, no `EntityRowLink`, no `EntityCreateButton`). Bring them onto the list substrate + wire the per-site "manage" detail through `defaultDetailView`, and route the New-site wizard through `EntityCreateButton` instead of its self-owned Radix modal. → [[list-substrate]], [[surface-frame]].
