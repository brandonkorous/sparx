...
**Why the heavyweight-form worry is self-solving:** a 287-line product form in a drawer would be cramped — but that same product's _detail_ already renders in that drawer, and the modal is `min(1200px, 94vw)`. A tenant who wants room sets `defaultDetailView: fullPage`. The preference is the pressure valve, so we never special-case by entity weight.
