...
This document is the implementation contract for the **Site Builder** (the `site`
...
all rendered by the existing tenant-aware site.

### What already exists (consume, do not rebuild)

| Capability                | Where                                                                                      | Site Builder relationship                                                                        |
| ------------------------- | ------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------ |
| TipTap content pages      | `Page` model (10-cms-pages), CMS dashboard, site `[...slug]` + `PageView`                  | CMS keeps prose pages; Site Builder owns _section-composed_ pages                                |
| Media library             | `MediaAsset` (15-cms-media), `mediaUrl()`, CMS media-picker                                | Reused for logos / section images                                                                |
| Navigation menus          | `NavigationMenu`/`NavigationItem` (16-cms-navigation), CMS menu-editor (`/cms/navigation`) | Owned by CMS; Site Builder binds a menu into a layout slot by id (read-only); site consumes them |
| Redirects, preview tokens | `Redirect`, `PreviewToken`, `?sparxPreview=`                                               | Reused for draft preview                                                                         |
| Limited theme tokens      | `SiteTheme` (47-commerce-site), `themeToCss()`                                             | **Write-through target** — see §4                                                                |
...
draft/publish/schedule/rollback lifecycle, light/dark theming, the site section &
...
- **Commerce owns `SiteTheme`.** Site Builder never adds columns to
  `commerce_site_themes`; it only upserts a _derived projection_ of the published
...
- **CMS owns navigation menus; Site Builder owns the layout slots.** The
  ...
  endpoints are module-neutral and unchanged (the site consumes the same rows); table
...
one `withTenant` transaction. The site's current `themeToCss()` `:root` injection is
unchanged. The **dark** map and richer detail travel via the public read endpoint (§6).

Rationale for write-through over superseding or extending `SiteTheme`: it keeps the
site token read path untouched ("deploy small"), preserves the commerce-owned
...
The site renders an ordered, visible-filtered section list through
...
edit ─▶ draftSettings / SiteSection rows (autosave, ETag-guarded)
          ...
          ▼
       SiteVersion N (immutable)  ──compile──▶ SiteTheme (light tokens, write-through)
...
- **Public read:** `GET /v1/public/site/site?tenant=<slug>` returns the published
...
The site layout injects `:root { … }` (light, via write-through) **and** a
...
Both can produce a site page; they are different tools and must never present two
competing "Pages" UIs:
...
editor; the site catch-all renders whichever exists for a slug, with sections
...
libs → service package → REST routes → scheduled publish → public endpoint → site
