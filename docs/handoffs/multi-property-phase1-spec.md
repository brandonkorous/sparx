...
| ---------------------------------------- | --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Entity name**                          | **`Property`** (table `properties`, column `property_id`) | Not "Site" — collides with the legacy `sitebuilder_*` `Site*` tables during the transition. Not "Storefront" — content-and/or-commerce, bigger than a store. Not "Channel"/"Surface" — both already taken (`channelsEnabled`, `@sparx/surface`). "Web property" is the industry term and scales from microsite → storefront → publication. |
...
| **StorefrontSettings / StorefrontTheme** | **Deferred**                                              | Commerce-owned; the Builder path themes off Surface CSS + `TenantBrand`, not `StorefrontTheme`. With a single primary property in Phase 1, per-property currency/theme is unobservable. Re-key when the second-property + per-property-theming UX lands.                                                                                   |
