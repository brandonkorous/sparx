...
- [x] Confirm the site draft path (`wizeworks/apps/site/lib/content.ts` + `site.ts`) returns the draft for a valid token — expect **no** site change.
...
- [ ] Rewire `resolveEmailBrand` (`wizeworks/packages/email-platform/src/services/brand-service.ts`) to read the tenant brand directly; drop the SiteTheme + `brandingOverride` branches; keep the sparx-default fallback.
...
- [x] **Brand content colors (DONE 2026-05-31; gate-green) — accent-content + secondary (+ its content).** Added
      ...
      forwarding, clear-via-null) + the `@wizeworks/site-themes` **v2** compile branch (`TenantBrandColumns` +
...
- [x] **S0 · Sample-data preview (doc 36 §9)** — DONE + green + runtime-verified (2026-05-31). Always-on,
      site-only, no schema change. NEW `wizeworks/apps/site/lib/sample-data.ts` (`SAMPLE_PRODUCT` +
...
- [x] **P-A · Rename (doc 36 §5) — DONE + green (2026-05-31).** `SiteTemplate`→`PageLayout` end to end as a
      **code-only rename, NO migration**: the Prisma model + field + all code symbols rename, but the physical
      ...
      Renamed: schema (`49-sitebuilder.prisma`, `02-tenant.prisma` `siteTemplates`→`pageLayouts`) + `@wizeworks/db`
...
- Every migration: authored locally against docker Postgres, applied via the DB Migrate workflow (never the laptop) — RLS hand-edited into the SQL. See [wizeworks/packages/db/README.md](../../packages/db/README.md).
