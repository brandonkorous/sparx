...
TENANT ─────────────  one organization — merchant, publisher, agency, team (Better Auth org, RLS tenant_id)
  ...
  └─ SITE ───────────  the website itself: themes, layouts, pages, sections, nav slots
       │               built by the **Builder** module — the site builder (module key `builder`)
...
1. **The site is the base, not the shop.** The website exists with zero commerce. Selling is
   the **Commerce** module, layered on. A tenant on Builder + CMS has a complete site with
   no shop. The site itself comes from the **Builder** module — without Builder there is no
   website, only headless data (CMS / CRM / Commerce via API + MCP).
...
| **Brand**                       | Tenant-level **identity**: name, logo, palette, type, shape, rhythm, effect (depth). Read by every surface; tenant brand is the **default**, optionally overridden **per site** (`Property.brand_override`, presentation-only — see the §2 amendment).                                                                                                                                                                   | A theme. A commerce-only concept.                                                                                   | `TenantBrand` (default) + `Property.brand_override`; [30 §6](30-sitebuilder-redesign.md), [33](33-token-model-v2.md), [49 §3](archive/49-multi-site-per-tenant.md) |
| **Site**                        | The tenant's **website** — the base layer: themes, layouts, pages, sections, nav. The output of the **Builder** module.                                                                                                                                                                                                                                                                                                  | The shop. (Commerce is separate.) A separate hidden website under the site — there is none.                         | `SiteConfig`/`SiteVersion`/`SiteSection`/`SiteLayoutBlock`; `49-sitebuilder.prisma`                                                                                |
| **Builder (module)**            | The paid module whose job is "site builder, themes, pages, live in 5 min." The base **website** capability. Its public name is **Builder** (module key `builder`, renamed from `storefront` before the store→site pass; a legacy `storefront` module-color alias still survives for the `/sitebuilder` surface). **Without it the tenant has no Site; sparx runs headless — CMS, CRM, and Commerce via API + MCP only.** | Commerce. The shop.                                                                                                 | `isModuleEnabled(tenantId, 'builder')`; [01 §2](01-platform-vision.md)                                                                                             |
| **Site Builder**                | The dashboard **tool** that authors the site (one-screen editor, theme, brand, layouts, sections). The admin-side surface of the Site module.                                                                                                                                                                                                                                                                            | The rendered public site.                                                                                           | `apps/dashboard/.../sitebuilder`; `wizeworks/packages/sitebuilder`                                                                                                 |
| **`wizeworks/apps/site`**       | The runtime **app** that renders the tenant's public website (tenant-aware, draft/published).                                                                                                                                                                                                                                                                                                                            | The admin/editor. The dashboard.                                                                                    | `wizeworks/apps/site`                                                                                                                                              |
...
| **Theme**                       | A complete look a site wears — one of the forty sparx ships as code (`FIRST_PARTY_THEMES`), or a tenant/partner upload — plus the tenant's brand and presentation overlays. How the site looks. (The six legacy presets it once meant — apex, industrial, drift, market, fleet, drop — are deleted.)                                                                                                                     | Brand/identity.                                                                                                     | `@wizeworks/silica-catalog` (the themes); `wizeworks/packages/site-themes` (the compiler); `CommerceSiteTheme`                                                     |
...
| **Module**                      | An independently activatable capability (Builder, Commerce, CMS, CRM, Email, B2B, AI/MCP, Dropship). Feature-flagged, not separately deployed.                                                                                                                                                                                                                                                                           | A microservice. A deploy unit.                                                                                      | module flags; `CLAUDE.md`                                                                                                                                          |

---

## 4. The website is the base, stated plainly

The **website** appears at several levels — and at every level it is **"site," never "store."**
The module that _builds_ it is **Builder**:

1. **Module** — **Builder** (key `builder`) = the website-building capability (base layer).
2. **Admin tool** — the Site Builder authors that module's data.
3. **Runtime app** — `wizeworks/apps/site` renders it.
4. **Public site** — `acme.sparx.zone`, the visitor-facing website.

**None of these mean "commerce / the shop."** Selling is always the separate **Commerce**
module. There is no "store" in the platform vocabulary — the tenant has a **site** (built by the
**Builder** module), and Commerce is a module on it. **Turn Builder off and the tenant has no
website at all — sparx runs headless, with CMS, CRM, and Commerce reachable only through the API
and MCP.**
...
| CSS class prefix `sf-*` + token vars `--sf-*` (`@sparx/site-ui`) | **`st-*` / `--st-*`** (`sx-` is taken by `@wizeworks/ui`) |
