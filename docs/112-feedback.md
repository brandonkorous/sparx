...
**RLS note** ([packages/db/CLAUDE.md](../packages/db/CLAUDE.md)): all three are tenant-scoped, so all
...
Both ride the canonical `@sparx/events` registry and the standard envelope
...
with a new **`feedback-response`** template in [packages/email/src/templates/](../packages/email/src/templates/).
