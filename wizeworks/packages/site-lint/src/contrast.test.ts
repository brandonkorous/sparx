...
describe("the theme's own color pairs", () => {
  ...
  it('reads lightness off the token, not off a round-tripped color', () => {
    ...
    // 4.1:1 on a legible color. One rounding error, one wrong verdict.
    //
...
describe("the author's own pairings", () => {
  ...
  it('inherits the background and the text color from ancestors', () => {
    const nested = body(
  ...
  it('computes the soft tint rather than treating it as the solid color', () => {
...
describe('without a theme', () => {
  it('returns no color findings at all rather than assuming a palette', () => {
...
describe('silicaui preset themes', () => {
  it('parses every token in every preset', () => {
    // A preset writes OKLCH. If `parseColor` ever stops handling it, every color
    ...
    for (const theme of THEME_PRESETS) {
      ...
      // gets right by construction. If OKLCH ever stops parsing, every color reads
