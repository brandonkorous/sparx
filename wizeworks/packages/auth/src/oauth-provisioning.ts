...
export async function finalizeOAuthSignup(input: {
  ...
  try {
    await authPrisma.$transaction(async (tx) => {
      ...
      // The BRAND's documents, read off the tenant that was just provisioned —
      // there is no request context in an OAuth hook, so the row is the only
      // place the brand can come from. See the same block in sign-up.ts for why
      // recording another brand's versions is worse than recording none.
      const tenant = await tx.tenant.findUnique({
        where: { id: input.tenantId },
        select: { platformBrand: true },
      });
      await tx.platformLegalAcceptance.createMany({
        data: acceptanceRows(tenant?.platformBrand).map((row) => ({
