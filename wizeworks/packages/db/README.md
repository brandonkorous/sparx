...
cp wizeworks/packages/db/.env.example wizeworks/packages/db/.env

# 3. Generate the Prisma client and apply migrations.
pnpm --filter @wizeworks/db db:generate
pnpm --filter @wizeworks/db db:migrate:deploy
...
1. **Author the migration locally** against the docker Postgres:
   ...
   pnpm --filter @wizeworks/db prisma migrate dev --name <descriptive_name>
   ```
...
3. **Commit + push to `main`.** The push triggers `DB Migrate` if any of these paths changed:
   - `wizeworks/packages/db/prisma/schema.prisma`
   - `wizeworks/packages/db/prisma/migrations/**`
   - `wizeworks/packages/db/sql/**`
   - `wizeworks/packages/db/scripts/run-migrations.ts`
   - `wizeworks/packages/db/Dockerfile`
