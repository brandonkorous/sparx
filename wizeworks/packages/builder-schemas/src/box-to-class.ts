...
// agent would type, and they compile through @wizeworks/surface-compile to the
...
// the plain color rows below carry only `btn-<color>`.
const LEGACY_BUTTON_STYLE_CLASS: Record<string, string> = {
  primary: 'btn-primary',
  soft: 'btn-primary btn-soft',
  dark: 'btn-neutral glass',
  // The frosted-light CTA. silica has no `surface` color, so the glass treatment
  ...
  // + the color set): a solid berry/secondary CTA the seed vocabulary can name.
  accent: 'btn-accent',
 ...
 *  `btn` base + color/treatment + the `md` size. */
