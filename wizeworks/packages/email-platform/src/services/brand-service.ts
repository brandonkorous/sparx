...
// @wizeworks/email threads through every template/atom).
//
...
// Public, cacheable media redirect (mirrors wizeworks/apps/site/lib/media.ts) — an
...
function resolveTenantLook(
  ...
  return withTenant(ctx, async (tx) => {
    ...
    // (colors/fonts) comes from the site's silica theme below. Computed up front —
    ...
    // tenant with NO identity at all yields null, and the caller renders @wizeworks/email's
    ...
    // counts: a tenant who set a name (but no colors/logo) keeps a branded email.
    const hasIdentity = [
      brand.businessName,
      brand.colorPrimary,
    ...
    // colors/fonts deliberately do NOT paint here: the look is the site's silica
