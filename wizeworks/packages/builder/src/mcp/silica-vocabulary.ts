...
// `@wizeworks/silica-catalog`, and the reconcile contract in
// `wizeworks/packages/builder/src/services/site-service.ts`. When those change, change this.
...
export const SILICA_STYLE_GUIDE = {
  ...
  blocksVsComposites: {
    ...
    colorBands:
      ...
      'comes from committing whole sections to a color and swinging between them (e.g. `bg-neutral` → `bg-primary` ' +
      '→ `bg-base-100` → `bg-accent`), which costs nothing and needs no photography. ' +
      'TRAP 1 — always pair a background with its ON-COLOR: `bg-neutral text-neutral-content`, ' +
      ...
      "TRAP 2 — HEADINGS DO NOT INHERIT THE BAND COLOR. silicaui's theme layer sets " +
      '`:where(h1,h2,h3,h4,h5,h6){ color: var(--color-base-content) }`, so a heading inside a dark band renders ' +
      'DARK-ON-DARK and effectively disappears, even though the paragraph beside it is fine. Put the on-color ' +
