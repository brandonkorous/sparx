...
export function create(ctx: ServiceContext, input: unknown): Promise<ThemeDto> {
  ...
  return withTenant(ctx, async (tx) => {
    ...
    // The look as it arrived is its first version, so a history is never empty for
    // something that plainly has a state.
    await captureThemeVersionTx(tx, ctx, row.id, parsed.theme, 'save');
    return toDto(row);
  });
}
...
export function update(ctx: ServiceContext, id: string, input: unknown): Promise<ThemeDto> {
  ...
  return withTenant(ctx, async (tx) => {
    ...
    // Sealed on every save. A rename alone produces identical tokens, shares a hash
    // with the newest version and gets no row — so the history lists changes to the
    // LOOK rather than every time the row was written.
    await captureThemeVersionTx(tx, ctx, row.id, row.draftTokens, 'save');
    return toDto(row);
  });
}
...
export function duplicate(ctx: ServiceContext, id: string, name?: string): Promise<ThemeDto> {
  return withTenant(ctx, async (tx) => {
    ...
    // A copy starts its OWN history at the state it was copied from. Sharing the
    // original's would let a restore on one silently rewrite the other.
    await captureThemeVersionTx(tx, ctx, row.id, source.draftTokens, 'save');
    return toDto(row);
  });
}
...
export function publish(ctx: ServiceContext, id: string): Promise<ThemeDto> {
  return withTenant(ctx, async (tx) => {
    ...
    // Marked in the history as the moment it went live, which is the entry someone
    // hunting for "what did it look like when we launched" is looking for.
...
export function usages(
  ctx: ServiceContext,
  id: string
): Promise<{ propertyId: string; name: string }[]> {
  return withTenant(ctx, async (tx) => {
    const rows = await tx.builderSite.findMany({
      where: { tenantId: ctx.tenantId, OR: [{ themeId: id }, { publishedThemeId: id }] },
      // The NAME, not just the id. This answers "what am I about to repaint", and an
      // id answers nothing a business owner can act on — they would have to go and
      // look it up somewhere else to find out whether they minded.
      select: { propertyId: true, property: { select: { name: true } } },
    });
    return rows.map((row) => ({ propertyId: row.propertyId, name: row.property.name }));
  });
}
