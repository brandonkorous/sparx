...
import type { BuilderLayout, BuilderPage, BuilderSite } from '@wizeworks/db';
...
// body; every other kit helper stays in @wizeworks/silica-catalog / the MCP tools.
...
/**
 * Write ONE page's body — one row, one UPDATE, nothing else read or written.
 *
 * This is the per-document editor's Save, and the narrowness is the point rather
 * than an optimisation. `setPageRoot` below does a read-modify-write of the WHOLE
 * site and hands the spliced result to `sync`, which upserts every page in the
 * roster. Two panes saving two different pages inside that window therefore lose
 * work: the second write carries the first page's tree as it stood when the second
 * pane read it, and puts the stale copy back over a save that already landed.
 *
 * Deletion was never the exposure — `pagesToDelete` refuses to infer a removal from
 * absence — so this was invisible in every test that asked whether a page survived.
 * What it costs is the CONTENT of a page nobody was looking at.
 *
 * The draft version is still sealed, so the overwritten state is recoverable in the
 * history either way; this stops it needing to be.
 */
export function writePageRoot(
  ctx: PropertyContext,
  pageId: string,
  root: SilicaNode
  ...
  return withTenant(ctx, async (tx) => {
    const updated = await tx.builderPage.updateMany({
      where: { id: pageId, propertyId: ctx.propertyId },
      data: { silicaDraftTree: asJson(root) },
    });
    // No such page — a stale pane, not a fault. The caller says so.
    if (updated.count === 0) return null;

    await reindexTreeTx(tx, ctx, { ownerKind: 'page', ownerId: pageId, tree: root });
    // Sealed exactly as `sync` seals one, so this page's history is unbroken by
    // the narrower path.
    await captureDraftVersionTx(tx, ctx, 'save');
    // No relay: a whole-root replace has no live delta op, and a co-editor
    // reloads this page — which is what the hint says.
    return { relay: null, reloadHints: [pageId] };
  });
}

/**
 * Write the ACTIVE layout's chrome — one row, one UPDATE.
 *
 * The layout's counterpart to `writePageRoot`, and narrow for the same reason: the
 * whole-site path would carry every page's tree back with it, so saving the header
 * while another pane saves a page could put that page's stale body back.
 */
export function writeFrameRoot(ctx: PropertyContext, root: SilicaNode): Promise<SilicaWriteChange> {
  return withTenant(ctx, async (tx) => {
    const layout = await activeLayoutTx(tx, ctx);
    await tx.builderLayout.update({
      where: { id: layout.id },
    ...
    await captureDraftVersionTx(tx, ctx, 'save');
    return { relay: null, reloadHints: ['frame'] };
  });
}
...
// `@wizeworks/blueprints`' `captureSite`, so a captured collection template keeps
// binding to its record type on reinstall.

/** One page as the capture path needs it — the source silica body `root` PLUS the
 *  domain columns. Structurally mirrors `@wizeworks/blueprints`' `CapturedPageInput` so
