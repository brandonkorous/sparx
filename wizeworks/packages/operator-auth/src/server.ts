    ...
    databaseHooks: {
      user: {
        update: {
          ...
          // (@wizeworks/auth server.ts): `twoFactorEnabled` flips only via
...
// Lazy Proxy (mirrors @wizeworks/auth): construct on first property access, not at
