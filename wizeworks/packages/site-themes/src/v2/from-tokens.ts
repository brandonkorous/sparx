...
// (`FIRST_PARTY_THEMES`, @wizeworks/silica-catalog). The tenant compile path speaks v2
...
// produced the same colors. The theme was real, shipped, and unreachable.
//
// Deliberately takes plain `Record<string, string>` maps rather than a silicaui
// `Theme`, so @wizeworks/site-themes keeps its zero runtime dependencies and the silica
 ...
 *  the storefront feeds it to the webfont loader (wizeworks/apps/site/app/layout.tsx reads
...
/** One mode's color slots. `dark` is read as a DELTA over `light` — silica stores
 *  it that way (colors only, shape/type live once), so an unstated dark slot
 ...
 * and presentation over. `dark` is the theme's dark color delta (silica's `Theme.dark`);
