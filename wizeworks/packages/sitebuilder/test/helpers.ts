// Per-test fixtures for @wizeworks/sitebuilder integration tests. Mirrors
// wizeworks/packages/crm/test/helpers.ts: every test gets a fresh tenant + staff user,
...
export async function createTestTenant(role = 'owner'): Promise<TestTenant> {
  ...
  // real tenant is born with a primary property (wizeworks/packages/auth sign-up); the
 ...
 *  (`wizeworks/apps/site/app/layout.tsx` reads the published copy of this and nothing else).
