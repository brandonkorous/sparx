...
import { SelectThemeInput, UpdateSettingsInput } from '@wizeworks/sitebuilder-schemas';
...
export interface SelectThemeResolution {
   ...
   *  not a column (docs/85 §6/§7) — the @wizeworks/sitebuilder package never reaches
   *  storage directly. */
...
export async function selectTheme(
  ...
  const updated = await withTenant(ctx, async (tx) => {
    ...
    // recolored the builder canvas (which derives its preview from these very
...
// Property `brand_override` — mirroring the color/font path
