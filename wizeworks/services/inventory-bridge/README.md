...
```bash
cp wizeworks/services/inventory-bridge/.env.example wizeworks/services/inventory-bridge/.env   # then fill it in
pnpm --filter @wizeworks/inventory-bridge start    # node --import tsx src/index.ts
# or, containerized:
docker build -f wizeworks/services/inventory-bridge/Dockerfile -t sparx-inventory-bridge .
