...
// `@wizeworks/db` — its client is constructed at import time and reads the pool size
// from the `connection_limit` query param on DATABASE_URL (wizeworks/packages/db/src/client.ts).
// This module has no `@wizeworks/db` dependency, so importing it FIRST in index.ts
// guarantees the rewrite lands before the client exists.
//
// WHY THE FLOOR EXISTS. The platform tick pattern `withAdvisoryTickLock`
// (wizeworks/packages/db/src/advisory-tick-lock.ts) deliberately PARKS one connection on the
