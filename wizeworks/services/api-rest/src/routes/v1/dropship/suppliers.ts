...
import { notFound, badRequest, conflict } from '@wizeworks/api-core/errors';
...
// Per-image cap — a many-angle catalog (Printify ships ~7 mockups per color)
...
//     color mockup across every size, so the same URL recurs across a color's
...
async function attachDropshipImages(
  ...
  // that references it (a color mockup shared by all its sizes → one asset).
...
// rows an imported multi-variant product shows NO size/color picker and silently
// strands the buyer on the default variant. We rebuild the lattice in first-seen
// order (so sizes/colors render in the sequence the supplier listed them) and
  ...
  for (const [i, name] of optionNames.entries()) {
    const option = await tx.productOption.create({
      data: {
        ...
        // No hex from suppliers, so color options render as labelled chips
...
async function createCommerceProductFromDropship(
  ...
  // value set — otherwise the storefront renders no size/color picker.
    ...
    variants.map((v, idx) => {
      ...
      return tx.productVariant.create({
        data: {
          ...
          // computes inStock=false and greys just that color/size out.
...
const dropshipSupplierRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post('/v1/dropship/suppliers/:id/catalog/:productId/reimport', async (request, reply) => {
    ...
    const result = await withTenant({ tenantId }, async (tx) => {
      ...
      // image fix gets its color-specific photos through the normal UI.
      ...
      // the storefront shows no size/color picker. Only when the product has
