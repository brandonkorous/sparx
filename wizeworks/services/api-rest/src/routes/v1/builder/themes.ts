...
//   GET    /v1/builder/themes/:id/versions     → this look's own history
//   POST   /v1/builder/themes/:id/versions/:versionId/restore
//                                              → put its DRAFT back to one
//
...
// @wizeworks/builder-schemas dependency.

import type { FastifyPluginAsync } from 'fastify';
import { z } from 'zod';
import { themeService, themeVersionService } from '@wizeworks/builder';
import { SPARX_THEME_GROUPS } from '@wizeworks/silica-catalog';
...
const builderThemeRoutes: FastifyPluginAsync = (app) => {
  ...
  // A look is TENANT-wide, so its history is its own table rather than an entry in
  // the property-scoped site snapshots — see `theme-version-service`.
  app.get('/v1/builder/themes/:id/versions', async (request) => {
    requireRole(request, 'viewer');
    await requireBuilderModule(request);
    const { id } = IdParam.parse(request.params);
    const { limit } = request.query as { limit?: string };
    const versions = await themeVersionService.listThemeVersions(
  ...
  // The DRAFT only. What visitors see moves when the look is published and each
  // site publishes, so this changes nothing anyone outside the console can see.
  app.post('/v1/builder/themes/:id/versions/:versionId/restore', async (request) => {
    requireRole(request, 'editor');
    await requireBuilderModule(request);
    const { id, versionId } = VersionParam.parse(request.params);
    return ok(
      await themeVersionService.restoreThemeVersion(toBuilderTenantContext(request), id, versionId)
    );
  });
