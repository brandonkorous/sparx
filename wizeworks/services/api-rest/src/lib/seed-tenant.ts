...
export interface SeedTenantSpec {
  ...
  sampleData?: boolean;
  /**
   * Which PRODUCT this demo tenant signed up under — `sparx` or `piggles`.
   *
   * Passed straight through to `provisionTenant`, which has taken it since
   * Piggles became a second brand; it was simply never exposed here, so every
   * seeded tenant fell back to the brand the seeding PROCESS serves. That
   * default is right for a running account app and wrong for a script: a demo
   * tenant seeded from the sparx API for the Piggles console came out
   * `platformBrand: 'sparx'` and never appeared in it.
   *
   * Omit for sparx — the fallback is unchanged, so no existing spec moves.
   */
  platformBrand?: string;
  /**
   * The zone the always-on `<slug>.<zoneDomain>` subdomain is created under.
   * Rides with `platformBrand` because it is the same mistake in the other
   * half of the row: Piggles tenants live on `piggles.site`, sparx on
   * `sparx.zone`, and one process serves both. Omit to keep the brand's own.
   */
  zoneDomain?: string;
}
...
async function findOrProvisionTenant(spec: SeedTenantSpec): Promise<ProvisionedTenant> {
  ...
  return prisma.$transaction(async (tx) => {
    const { tenantId, propertyId } = await provisionTenant(tx, {
      ...
      // Both undefined for every sparx spec, so provisioning falls back to the
      // brand this process serves exactly as before.
      platformBrand: spec.platformBrand,
      zoneDomain: spec.zoneDomain,
    });
