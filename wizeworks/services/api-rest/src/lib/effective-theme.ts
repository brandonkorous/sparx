...
// know what colors a visitor gets. The legacy-tier retirement needs the SAME answer
 ...
 * derivation the builder canvas opens on (it moved into `@wizeworks/site-themes` so both
 * read one copy), including the per-site brand override that makes a second site under
 * one tenant look like itself.
 *
 * Returns null if every path fails. For the Check that is not a failure — the color
...
export async function effectiveTheme(
  ...
  // look degrades to the tenant's own colors rather than to nothing.
  ...
  const compiled = tenantTheme(applyBrandOverride(base, property?.brandOverride), {
    ...
    // The site's OWN theme, so the caller judges the colors the visitor gets. Without
