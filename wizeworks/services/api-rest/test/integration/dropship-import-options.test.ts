...
import { type Prisma, prisma, withTenant } from '@wizeworks/db';
...
// `shared.png` URL recurs across every variant (as a color mockup would across
...
describe('dropship import → product option lattice', () => {
  ...
  // A supplier (Printify) flips an individual color/size combo out of stock via
  ...
  it('imports an unavailable combo as deny while the rest stay orderable', async () => {
    ...
    try {
      ...
      const dpId = await seedDropshipProduct(t, supplierId, 'PP-AVAIL', [
        colorVariant('AV:red', 'Red', true),
        colorVariant('AV:blue', 'Blue', false), // supplier currently can't make it
        colorVariant('AV:green', 'Green'), // flag absent → available
      ]);
      ...
      // Other colors are orderable, so the product itself stays in stock.
  ...
  it('marks the product out of stock when every combo is unavailable', async () => {
    ...
    try {
      ...
      const dpId = await seedDropshipProduct(t, supplierId, 'PP-AVAIL-NONE', [
        colorVariant('AN:red', 'Red', false),
        colorVariant('AN:blue', 'Blue', false),
      ]);
  ...
  it('re-sync re-applies changed availability both ways (deny ⇄ continue)', async () => {
    ...
    try {
      ...
      const supplierId = await seedSupplier(t);
      const allAvailable = [
        colorVariant('RS:red', 'Red', true),
        colorVariant('RS:blue', 'Blue', true),
      ];
      ...
      await withTenant({ tenantId: t.tenantId }, (tx) =>
        tx.dropshipProduct.update({
          where: { id: dpId },
          data: { variants: [colorVariant('RS:red', 'Red', false), allAvailable[1]!] as object[] },
        })
      );
