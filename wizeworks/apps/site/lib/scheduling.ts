...
// shape. Tenant is resolved from the request host (verifiedSiteRoute — the
// brand-checked form, so a zone host can never reach another brand's tenant); a tenant
// without the scheduling module active returns null/[] so the /book route 404s.

import { resolveActivePropertySlug, verifiedSiteRoute } from './site-context';
...
async function publicGet<T>(path: string, init?: RequestInit): Promise<T | null> {
  const route = await verifiedSiteRoute();
...
export async function activeTenantSlug(): Promise<string | null> {
  return (await verifiedSiteRoute())?.tenantSlug ?? null;
}
