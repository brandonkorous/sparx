# Drop-in prompt — build a workbench surface

Copy everything below the line. Change **only** the `FEATURE` block at the top.
...
- API it consumes: `<GET/POST /v1/... — or "find it">`
- Anything already stubbed for it: `<file:line, or "a stub() entry in lib/surfaces/catalog/platform.ts">`

Everything below is fixed. Do not treat it as suggestions.
