// Forgot password — an EDITABLE shell around the PINNED `commerce.auth` core in `forgot`
// mode (docs/122). The live request-a-reset-link form lives in the host node the tenant can
...
export default async function ForgotPasswordPage() {
  ...
  const published = await getPublishedSilicaPage(site.slug, 'account/forgot');
  const shell =
    published?.root ?? functionalShell(HOST_KEYS.commerceAuth, { props: { mode: 'forgot' } });
