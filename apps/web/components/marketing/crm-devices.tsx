                ...
                <div className="min-w-0 flex-1">
                    <Panel header="the usual stack" headerDot={BROKEN}>
                        {before.map((row, i) => (
                            <div
                                key={row.label}
                                className={`flex items-center gap-[11px] px-5 py-[13px] text-sm ${i === 0 ? '' : 'border-base-200 border-t'
                                    }`}
                            >
...
function Panel({
    ...
    return (
        <div
            className={`bg-base-100 border-base-300 h-full overflow-hidden rounded-2xl border-x border-b ${accent ? 'border-t-module-crm border-t-[3px]' : 'border-t'
                }`}
        >
