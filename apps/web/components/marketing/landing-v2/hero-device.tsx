...
/** Window-chrome traffic lights — device mimicry. The three hues are the OS
 *  convention (close / minimise / zoom), and the semantic tokens already carry
 *  exactly that red-amber-green triad, so they are spelled as tokens rather than
 *  as Tailwind's own palette. They are 8px decorative dots with no label, which
 *  is why a status token is the right fit and not a misuse of one. */
const TRAFFIC_LIGHTS = ['bg-error', 'bg-warning', 'bg-success'];
  ...
  return (
    <div className="relative">
      <div className="bg-base-100 overflow-hidden rounded-[22px] shadow-2xl">
        ...
        <div className="flex flex-col gap-[22px] p-6">
          <div className="flex items-start justify-between">
            <h3 className="m-0 text-xl font-medium tracking-[-0.02em]">Good morning, Jess.</h3>
            {/* Status signal — silica's own `soft` wash, not a hand-rolled color-mix. */}
            <span className="bg-success bg-soft text-success rounded-full px-2.5 py-1.5 text-sm font-medium whitespace-nowrap">
              Business online
            </span>
          </div>
          ...
          <div className="flex flex-col gap-2.5">
            ...
            <span className="text-sm font-medium">Handled by sparx</span>
      ...
      <div className="mkt-hide-on-mobile bg-secondary text-secondary-content absolute right-[-28px] bottom-9 w-[150px] rotate-[-4deg] rounded-2xl p-4 shadow-2xl">
        <span className="block text-sm">This week</span>
        <span className="my-0.5 block text-[28px] font-bold tracking-[-0.03em]">+24%</span>
        <span className="block text-sm">online orders</span>
      </div>
    </div>
  );
}
...
function DemoAction({ label, sub }: { label: string; sub: string }) {
  return (
    <div className="bg-base-100 border-base-300 rounded-[10px] border p-3 text-left">
      ...
      <span className="mt-0.5 block text-sm">{sub}</span>
    </div>
  );
}
