// One source for how a partner's specialty is labelled AND colored.
//
...
// color, so every specialty tag rendered the same grey `neutral` pill: four
...
// its own marketing page. That makes the directory scannable by color: a
...
export interface Specialty {
    label: string;
    /** A registered silica color NAME, for `<Badge color=…>`. */
    color: string;
