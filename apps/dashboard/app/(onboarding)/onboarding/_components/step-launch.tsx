...
function TemplateLaunch({
  ...
  if (published) {
    return <LaunchSuccess storeUrl={storeUrl} slug={slug} />;
  }

  const previewSrc = token ? `${storeUrl}/?sparxSitePreview=${encodeURIComponent(token)}` : null;
...
function LaunchSuccess({ storeUrl, slug }: { storeUrl: string | null; slug: string }) {
  return (
