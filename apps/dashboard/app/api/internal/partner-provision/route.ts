...
// account (docs/114 §B.2). Better Auth lives only in this dashboard process, so
// api-rest — which runs the operator approval — cannot mint the tenant + owner
// login itself. It POSTs here instead, authenticated with the shared internal
// secret (SPARX_INTERNAL_JWT_SECRET, already shared dashboard↔api-rest for
// session passing), and we run the Better Auth provisioning + set-password invite.
//
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    process.stderr.write(
      JSON.stringify({
        severity: 'ERROR',
        source: 'dashboard.partner-provision',
