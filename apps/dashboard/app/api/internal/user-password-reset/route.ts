...
// Internal, service-to-service password reset for a staff user (docs/apps/admin —
// user & site management). Better Auth lives only in this dashboard process, so
// api-rest — which runs the operator console's user actions — cannot trigger a
// reset itself. It POSTs here instead, authenticated with the shared internal
// secret (SPARX_INTERNAL_JWT_SECRET, already shared dashboard↔api-rest), and we
// call auth.api.requestPasswordReset — which publishes the `password-reset` email
// via the sendResetPassword callback (packages/auth/src/server.ts), exactly as the
// self-service forgot-password flow does.
//
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    process.stderr.write(
      JSON.stringify({
        severity: 'ERROR',
        source: 'dashboard.user-password-reset',
