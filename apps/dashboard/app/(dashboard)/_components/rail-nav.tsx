...
function tileIconClass(active: boolean) {
    return `inline-flex h-4 w-4 shrink-0 items-center justify-center ${active ? 'text-module' : 'text-base-content/50 group-hover:text-base-content/70'
        }`;
}
...
interface RailNavProps {
     ...
     *  partner). The Partner tile is PARTNERS-ONLY: shown only when the tenant is a
     *  partner AND this user is authorized — non-partners join via the public
     *  sparx.works/partners application form, not a permanent sidebar tile. */
...
export function RailNav({
    ...
    return (
        <>
            ...
            >
                <Wordmark
                    icon={expanded ? false : true}
                    className={expanded ? 'h-[32px] w-auto' : 'h-[32px] w-[32px]'}
                />
            </div>
          ...
          above, Settings + the expand toggle stay pinned below. */}
            <div
                className={`flex min-h-0 w-full flex-1 flex-col gap-1 overflow-y-auto ${expanded ? 'items-stretch' : 'items-center'
                    }`}
            >
          ...
          the glyph always carries the partner color. PARTNERS-ONLY: the tile shows
          only when the tenant is a partner AND this user is authorized to operate
          it — non-partners join through the public sparx.works/partners
          application form, not a permanent tile in everyone's sidebar. */}
