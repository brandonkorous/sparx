...
interface B2bAccountCreateFormProps {
  surface: 'page' | 'overlay';
}

export function B2bAccountCreateForm({ surface }: B2bAccountCreateFormProps) {
  ...
  function onCreated(id: string) {
    if (surface === 'overlay') {
      ...
      next.delete('drawer');
      next.delete('modal');
      next.set(mode, `b2b-account:${id}`);
      router.replace(`${pathname ?? '/'}?${next.toString()}`);
      router.refresh();
      return;
    }
    router.push(`/crm/b2b/${id}`);
    router.refresh();
  }
  ...
  return (
    <Stack gap={6}>
      {surface === 'overlay' && (
        <Stack gap={1}>
          <Heading level={2}>New B2B account</Heading>
