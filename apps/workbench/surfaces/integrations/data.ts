...
// Integrations data — the outside services a business connects so information
// moves between them and sparx on its own (shipping carriers, card payments,
// sales tax, and so on).
//
// Built on the provider marketplace (@sparx/integration-framework) through the
// existing /v1/commerce/providers/* endpoints — the catalog of what CAN be
// connected, plus this business's own connections. Those endpoints are part of
// the selling features, so when a business has not turned selling on they answer
// MODULE_DISABLED; the surface detects that and explains it rather than erroring.
//
// This file owns every `['integrations', …]` query key and the shapes the two
// integration surfaces read. Reads are viewer; connecting, changing and
// disconnecting are admin — mirrored with useViewer so a control never appears
// for someone the server will refuse.
...
export const INTEGRATIONS_KEYS = {
  ...
  installations: ['integrations', 'installations'] as const,
  installation: (id: string) => ['integrations', 'installation', id] as const,
  metadata: (slug: string) => ['integrations', 'metadata', slug] as const,
};
...
export function useAvailableIntegrations() {
  return useQuery({
    queryKey: INTEGRATIONS_KEYS.available,
...
export function useInstallations() {
  return useQuery({
    queryKey: INTEGRATIONS_KEYS.installations,
...
export function useInstallation(id: string) {
  return useQuery({
    queryKey: INTEGRATIONS_KEYS.installation(id),
...
export function useProviderMetadata(slug: string | undefined) {
  return useQuery({
    queryKey: INTEGRATIONS_KEYS.metadata(slug ?? ''),
...
function useInvalidate() {
  ...
  return (id?: string) => {
    void queryClient.invalidateQueries({ queryKey: INTEGRATIONS_KEYS.installations });
    if (id) void queryClient.invalidateQueries({ queryKey: INTEGRATIONS_KEYS.installation(id) });
  };
}
 ...
 *  carries its own colour. `enabled` overrides the raw status — a paused
