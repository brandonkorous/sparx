// Cloud Run entrypoint. Pub/Sub pushes to POST / — we verify the OIDC token's
// `email` claim matches the expected invoker SA, dispatch to handle(), and
// respond 204 (ack) / 5xx (nack-and-retry). Mirrors services/legal-seed-worker.
//
...
async function handlePush(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  if (!event) {
    logger.warn({ messageId, raw: parsed }, 'message is not an event this worker handles; acking');
    res.statusCode = 204;
    res.end();
    return;
  }
  ...
  try {
    ...
    logger.info(
      { messageId, tenantId: event.tenantId, type: event.type, outcome },
      'message processed'
    );
    res.statusCode = 204;
    res.end();
    ...
    logger.error(
      { err, messageId, type: event.type },
      'transient mirror failure — returning 500 to trigger redelivery'
    );
    res.statusCode = 500;
    res.end();
  }
...
function main(): void {
  ...
  server.listen(env.PORT, '0.0.0.0', () => {
    logger.info({ port: env.PORT }, 'platform-crm-worker listening for Pub/Sub pushes');
  });
...
try {
  ...
  logger.fatal({ err }, 'platform-crm-worker failed to start');
  process.exit(1);
}
...
async function handleFromBroker(raw: string): Promise<void> {
  ...
  try {
    ...
    logger.error({ err }, 'platform-crm-worker: broker message not valid JSON; acking');
    return;
  }
  ...
  if (!event) {
    ...
    logger.warn(
      { raw: parsed },
      'platform-crm-worker: broker message did not match schema; acking'
    );
    return;
  }
...
void startConsumer({
  ...
  .catch((err: unknown) => {
    ...
    logger.fatal({ err }, 'platform-crm-worker: broker subscription failed');
    process.exit(1);
  });
