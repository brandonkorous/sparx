...
//
// Reads OTE vs production credentials based on NODE_ENV. All domain purchase,
...
function baseUrl(): string {
  return env.NODE_ENV === 'production' ? PROD_BASE : OTE_BASE;
}
...
// GoDaddy response shapes (internal — not exported to callers)
...
export function generateDkimKeypair(): { publicKey: string; privateKey: string } {
  ...
  // Strip PEM headers/footers + whitespace — the DKIM `p=` value is raw base64.
...
/**
 * Initiate a transfer-out for a domain the tenant wishes to take to another
 * registrar. Unlocks the domain then fetches the EPP auth code.
 */
