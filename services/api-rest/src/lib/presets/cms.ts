...
//
// These live at the api-rest composition root (not in a `@sparx/cms` package —
...
// tenant-scoped under the same RLS. The contract comes from @sparx/auth (api-rest
// already deps it), the field vocabulary from @sparx/cms-schemas.
//
...
import { ContentTypeSchema, type ContentTypeDefinition, type FieldDef } from '@sparx/cms-schemas';
import { definePreset, type ModulePreset } from '@sparx/auth';
import type { Prisma, TenantContext } from '@sparx/db';
...
// ─── Field helpers (each returns one FieldDef in the @sparx/cms-schemas union) ──
