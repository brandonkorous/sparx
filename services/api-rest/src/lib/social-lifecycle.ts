...
//
// Pub/Sub stays on the ROUTE, mirroring Slice 4's publish-now: these helpers do the DB
// transition and report which event the caller should emit (`due` / `scheduled`), so
// the request handler emits it with `request.log`. No publisher plumbing in here.
...
import { toPostView, type SocialPostView } from './social-posts.js';
