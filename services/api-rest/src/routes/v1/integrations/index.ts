...
//
// The catalog half comes from the shared plane (@sparx/integrations), so this route
...
const integrationRoutes: FastifyPluginAsync = async (app) => {
  app.get('/v1/integrations', async (request) => {
    ...
    // @sparx/integrations types.ts: a category earns its place by being dispatched.
