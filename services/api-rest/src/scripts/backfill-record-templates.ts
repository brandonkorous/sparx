...
//
//   pnpm --filter @sparx/api-rest ops:backfill-record-templates                    # dry run
//   pnpm --filter @sparx/api-rest ops:backfill-record-templates -- --apply
//   pnpm --filter @sparx/api-rest ops:backfill-record-templates -- --tenant=harbor-pine
//   pnpm --filter @sparx/api-rest ops:backfill-record-templates -- --type=commerce.product
//   pnpm --filter @sparx/api-rest ops:backfill-record-templates -- --include-fingerprint=4499a774a668
//
// ── WHY THIS EXISTS ────────────────────────────────────────────────────────────
...
// would report zero rows and exit looking successful (packages/db/CLAUDE.md).

import { prisma, withTenant, Prisma } from '@sparx/db';
import {
  RECORD_TEMPLATES,
  ROUTED_RECORD_TYPES,
