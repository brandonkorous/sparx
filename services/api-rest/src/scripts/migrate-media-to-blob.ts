...
//
//   pnpm --filter @sparx/api-rest ops:migrate-media-to-blob            # dry run
//   pnpm --filter @sparx/api-rest ops:migrate-media-to-blob -- --apply
//
// ── WHY THIS EXISTS ───────────────────────────────────────────────────────────
// Media used to live in GCS. When GCP was retired `GCS_MEDIA_BUCKET` went with it and
// every `getStorage()` — api-rest, media-worker, packages/media — fell through to its
