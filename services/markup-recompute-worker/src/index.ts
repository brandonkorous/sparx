...
async function handleFromBroker(raw: string): Promise<void> {
  ...
  try {
    ...
    logger.error({ err }, 'markup-recompute-worker: broker message not valid JSON; acking');
    return;
  }
  ...
  if (!event) {
    ...
    logger.warn(
      { raw: parsed },
      'markup-recompute-worker: broker message did not match schema; acking'
    );
    return;
  }
...
void startConsumer({
  durable: 'markup-recompute-worker',
  events: ['variant.cost.updated'],
  ...
  .catch((err: unknown) => {
    ...
    logger.fatal({ err }, 'markup-recompute-worker: broker subscription failed');
    process.exit(1);
  });
