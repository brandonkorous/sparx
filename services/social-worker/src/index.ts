...
async function handleFromBroker(raw: string): Promise<void> {
  ...
  try {
    ...
    logger.error({ err }, 'social-worker: broker message not valid JSON; acking');
    return;
  }
  ...
  if (!event) {
    ...
    logger.warn({ raw: parsed }, 'social-worker: broker message did not match schema; acking');
    return;
  }
...
void startConsumer({
  ...
  .catch((err: unknown) => {
    ...
    logger.fatal({ err }, 'social-worker: broker subscription failed');
    process.exit(1);
  });
