...
// respond 204 (ack) / 5xx (nack-and-retry). Mirrors services/email-worker.
//
...
async function handlePush(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  if (!event) {
    logger.warn({ messageId, raw: parsed }, 'message did not match tenant.created schema; acking');
    res.statusCode = 204;
    res.end();
    return;
  }
  ...
  try {
    ...
    logger.info(
      { messageId, tenantId: event.tenantId, created: outcome.created, skipped: outcome.skipped },
      'message processed'
    );
    res.statusCode = 204;
    res.end();
    ...
    logger.error(
      { err, messageId },
      'transient seed failure — returning 500 to trigger redelivery'
    );
    res.statusCode = 500;
    res.end();
  }
...
function main(): void {
  ...
  server.listen(env.PORT, '0.0.0.0', () => {
    logger.info({ port: env.PORT }, 'legal-seed-worker listening for Pub/Sub pushes');
  });
...
try {
  ...
  logger.fatal({ err }, 'legal-seed-worker failed to start');
  process.exit(1);
}
...
async function handleFromBroker(raw: string): Promise<void> {
  ...
  try {
    ...
    logger.error({ err }, 'legal-seed-worker: broker message not valid JSON; acking');
    return;
  }
  ...
  if (!event) {
    ...
    logger.warn({ raw: parsed }, 'legal-seed-worker: broker message did not match schema; acking');
    return;
  }
...
void startConsumer({
  ...
  .catch((err: unknown) => {
    ...
    logger.fatal({ err }, 'legal-seed-worker: broker subscription failed');
    process.exit(1);
  });
