...
//   pnpm --filter @sparx/api-rest exec tsx "$PWD/marketplace-catalog/_gen/gen-retail-jewelry-fine.ts"
    ...
    children: [
      el('div', 'mx-auto flex w-full max-w-xl flex-col items-center gap-5', {
        children: [
          ...
          el('p', 'text-lg leading-relaxed text-base-content', {
            text: 'A question about a piece, a custom commission, sizing, or a repair? Tell us what you’re after and a real jeweler — not a bot — will write you back.',
          }),
          el('a', 'btn btn-primary btn-lg', { attrs: { href: 'mailto:hello@aurelia.example' }, text: 'Email the workshop' }),
        ],
      }),
    ],
  }),
    ...
    body: {
      ...
      body: {
        ...
        content: [
          ...
          para('Warm skin tones tend to sit beautifully against gold; cooler tones against silver — but the honest answer is to hold each to your wrist in daylight and trust your eye. In wear, 14k gold barely changes: it keeps its colour for decades with almost no effort. Sterling silver naturally tarnishes to a soft grey over time, which some people love as patina and others polish back to bright in seconds with a cloth.'),
