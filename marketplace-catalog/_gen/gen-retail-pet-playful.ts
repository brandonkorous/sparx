...
// catalogue of a pop-color webbing collar and matching lead, a reversible bandana, a mega toy
...
//   pnpm --filter @wizeworks/api-rest exec tsx "$PWD/marketplace-catalog/_gen/gen-retail-pet-playful.ts"
...
const ASSETS: Asset[] = [
    ...
    { id: 'fetch-tile-walk', url: src('fetch-tile-walk'), alt: 'A happy dog in a bright pop-color collar on a walk' },
    ...
    { id: 'fetch-tile-rest', url: src('fetch-tile-rest'), alt: 'A dog flopped happily across a colorful bed' },
    { id: 'fetch-band-made', url: src('fetch-band-made'), alt: 'A wall of bright collars and toys in fun colors' },
    { id: 'fetch-band-sub', url: src('fetch-band-sub'), alt: 'A colorful treat box being torn open by an excited dog' },
    { id: 'prod-collar', url: src('prod-collar'), alt: 'A bright pop-color webbing dog collar' },
    { id: 'prod-lead', url: src('prod-lead'), alt: 'A matching pop-color clip lead coiled up' },
    { id: 'prod-bandana', url: src('prod-bandana'), alt: 'A reversible patterned dog bandana' },
    { id: 'prod-toybundle', url: src('prod-toybundle'), alt: 'A pile of colorful dog toys in a bundle' },
    ...
    { id: 'prod-bowl', url: src('prod-bowl'), alt: 'A roll-up silicone travel bowl in a fun color' },
                ...
                children: [
                    ...
                    el('div', 'grid grid-cols-2 gap-4 @3xl:grid-cols-4 @3xl:gap-6', {
                        children: [
                            ...
                            categoryTile({ assetId: 'fetch-tile-rest', label: 'Rest', href: '/shop', alt: 'A dog flopped across a colorful bed' }),
                        ],
                    }),
                ],
            }),
...
const HOME: Node[] = [
    ...
    editorialBand({
        ...
        alt: 'A wall of bright collars and toys in fun colors',
    }),
    ...
    editorialBand({
        ...
        alt: 'A colorful treat box being torn open by an excited dog',
    }),
];
...
/** A piece offered in a set of color swatches — one variant per color, flat price. */
const colored = (b: Base & { colors: string[]; optionName?: string }): Product => {
    const name = b.optionName ?? 'Color';
...
/** A piece offered across a Size × Color lattice — the cartesian product of both option
 *  lists at a flat price. Every variant sets BOTH options, exactly one is default, and the SKU
 *  pins the size + color codes so no two combinations collide. The blueprint validator
...
const sizeColor = (
    ...
    const colorName = b.colorOption ?? 'Color';
...
const COMMERCE = {
    categories: [
        { handle: 'walk', name: 'Walk', description: 'Pop-color collars, leads, bandanas and travel gear for every outing.', featured: true },
