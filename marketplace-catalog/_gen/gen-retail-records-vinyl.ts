...
// a hot-magenta primary and an electric-cyan accent — the album covers carry the color).
...
//   pnpm --filter @wizeworks/api-rest exec tsx "$PWD/marketplace-catalog/_gen/gen-retail-records-vinyl.ts"
...
// album covers supply the rest of the color. A characterful geometric display over a clean
...
// An LP is sold in two pressings — the standard black 180g and a limited color variant that
// carries a small premium. Standard is the default so the shelf price reads honestly.
const EDITION: OptionDecl = {
    name: 'Edition',
    displayType: 'dropdown',
    values: [{ value: 'Standard black 180g' }, { value: 'Limited color' }],
};
...
const lp = (opts: {
    ...
    variants: [
        ...
        { sku: `${opts.sku}-COL`, priceCents: money(opts.price + 5), inventoryPolicy: 'continue', optionValues: { Edition: 'Limited color' } },
    ],
...
const COMMERCE = {
    ...
    collections: [
        ...
        },
        {
            ...
            description: 'Synths, motorik pulses and color wax.',
            type: 'manual',
            featured: false,
