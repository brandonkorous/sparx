...
//   pnpm --filter @wizeworks/api-rest exec tsx "$PWD/marketplace-catalog/_gen/gen-retail-records-classic.ts"
...
// accent for links — the two colors that put a nostalgic album jacket on a shop wall. A
...
// An LP is sold in two pressings — the standard 180g black and a limited "Sunset" color
// pressing that carries a small premium. Standard is the default so the shelf price reads
// honestly.
const EDITION: OptionDecl = {
    name: 'Edition',
    displayType: 'dropdown',
    values: [{ value: 'Standard 180g black' }, { value: 'Limited Sunset color' }],
};
...
const lp = (opts: {
    ...
    variants: [
        ...
        { sku: `${opts.sku}-COL`, priceCents: money(opts.price + 6), inventoryPolicy: 'continue', optionValues: { Edition: 'Limited Sunset color' } },
    ],
