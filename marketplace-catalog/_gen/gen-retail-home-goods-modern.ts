...
//   pnpm --filter @wizeworks/api-rest exec tsx "$PWD/marketplace-catalog/_gen/gen-retail-home-goods-modern.ts"
...
const ASSETS: Asset[] = [
    ...
    { id: 'prod-cushions', url: src('prod-cushions'), alt: 'A pair of geometric color-blocked cushions on a bench' },
...
/** A crisp SPLIT editorial band — a square photo on one side and a SOLID color text block on
...
const SEARCH: Node[] = [
    pageMasthead('Search Form & Field', 'Looking for a color, a material, or a piece for a particular room? Search the whole shop and the journal below.'),
];
const CART: Node[] = [
...
/** A piece offered in a set of color/finish swatches — one variant per color, flat price. */
...
const PRODUCTS: Product[] = [
    ...
    colored({
        ...
        description:
            'A rectangular serving tray in hand-lacquered wood, with a low lip and a surface so smooth it reads as a single plane of color. Corrals the coffee things, carries drinks to the table, or lives on the ottoman as a place to set a room. Wipe-clean and quietly luxurious.',
        price: 74,
        sku: 'FF-TAB-PLANE',
