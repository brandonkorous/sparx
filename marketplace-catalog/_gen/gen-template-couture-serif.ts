...
// carries the ENTIRE colour palette — the chrome is pure black on white.
  ...
  el('section', 'bg-base-100 @container px-6 py-20 text-center', {
    children: [
      el('div', 'mx-auto flex w-full max-w-xl flex-col items-center gap-5', {
        children: [
          el('h1', 'text-4xl font-medium tracking-tight text-base-content @2xl:text-5xl', {
            text: 'Visit the atelier',
          }),
          el('p', 'text-lg leading-relaxed text-base-content', {
            text: 'Our atelier and salon are open by appointment. Tell us what you are looking for — a single piece, a bespoke commission, or a fragrance to be layered to your own — and we will find a time.',
          }),
          el('a', 'btn btn-primary btn-lg', {
            attrs: { href: 'mailto:atelier@verane.example' },
            text: 'Email the atelier',
          }),
        ],
      }),
    ],
...
const PRODUCTS: Product[] = [
  ...
  },
  {
    ...
    description:
      'An unbroken line of hand-set white diamonds on an 18k gold rail, each stone chosen to match its neighbour for cut and colour. The clasp is worked to sit flat and disappear.',
    status: 'active',
    ...
    body: {
      ...
      body: {
        ...
        content: [
          ...
          para('Every stone in a Colette bracelet is chosen against its neighbours for cut and colour, then grain-set by one setter from end to end. A machine could set faster; it could not match the line the way an eye does, and the line is the whole point.'),
          h2('Few by necessity'),
