...
/** Post metadata for the Journal INDEX page cards (pages/journal.ts) — the single
 *  source of slug/title/excerpt/seed shared with the seeded content below, so the
 *  index never drifts from the posts. Each card links to `/blog/<slug>`; the cover
 *  uses the same `seed` emoji/colour the post's featured image bakes. */
export const journalPosts = POSTS.map((p) => ({
  slug: p.slug,
  title: p.body.title,
