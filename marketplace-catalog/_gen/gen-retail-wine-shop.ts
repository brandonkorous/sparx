...
//   pnpm --filter @sparx/api-rest exec tsx "$PWD/marketplace-catalog/_gen/gen-retail-wine-shop.ts"
...
import { defineTheme, face, STATUS_ON_DARK } from '../../packages/silica-catalog/src/themes';
...
const HOME: Node[] = [
  ...
  editorialBand({
    ...
    lead: 'Natural wine is farmed organically, fermented with its own wild yeast, and bottled with little or no added sulphur — no lab yeasts, no colour, no fining agents, no filtering the life out of it. What that gets you is wine that tastes like the year, the place and the person who grew it.',
...
const SHOP: Node[] = [
  pageMasthead(
    'The whole shelf',
    'Everything we are pouring right now — reds, whites, skin-contact oranges, pét-nats, rosé, a magnum or two and a low-intervention non-alc. Filter by colour or sort however you like; every bottle here is farmed clean and made with a light hand.'
  ),
];
...
const SEARCH: Node[] = [
  pageMasthead('Search Sediment', 'Looking for a grape, a colour, a region or a pairing? Search the whole shop and the journal below.'),
];
const CART: Node[] = [
  ...
  el('section', 'bg-base-100 @container px-6 py-20 text-center', {
    children: [
      el('div', 'mx-auto flex w-full max-w-xl flex-col items-center gap-5', {
        children: [
          el('h1', 'text-5xl font-bold tracking-tight text-base-content @2xl:text-6xl', { text: 'Come say hello' }),
          el('p', 'text-lg leading-relaxed text-base-content', {
            text: 'After a bottle for a dinner, a case for a party, or a hand picking something for a gift? Tell us what you are after and a real person from the shop will get back to you — recommendations always welcome.',
          }),
          el('a', 'btn btn-primary btn-lg', { attrs: { href: 'mailto:hello@sedimentwine.example' }, text: 'Email the shop' }),
        ],
      }),
    ],
  }),
...
const PRODUCTS: Product[] = [
  ...
  bottle({
    ...
    description:
      'A dry, pale rosé with none of the sugary softness the colour gets blamed for. Wild strawberry, pink grapefruit and a herbal, savoury edge, with real texture underneath. The house rosé from the first warm day to the last.',
    price: 23,
  ...
  },
  {
    ...
    description:
      'Six bottles, our pick, at a better price than buying them one by one — the easiest way to fill the rack with low-intervention wine you will actually finish. Choose a mix of colours, all reds, or all whites and oranges; we swap the line-up every month.',
    status: 'active',
    ...
    options: [
      { name: 'Selection', displayType: 'dropdown', values: [{ value: 'Mixed colours' }, { value: 'All reds' }, { value: 'Whites & oranges' }] },
    ],
    variants: [
      { sku: 'SEDIMENT-SIX-MIX', priceCents: money(135), isDefault: true, inventoryPolicy: 'continue', optionValues: { Selection: 'Mixed colours' } },
    ...
    body: {
      ...
      body: {
        ...
        content: [
          ...
          h2('In the cellar'),
          para('Then, as little intervention as the wine allows. Fermentation with the wild yeast that arrives on the grapes, rather than a packet of lab yeast chosen for a predictable flavour. No added acid, colour, tannin or sugar. No fining or heavy filtering to strip it clear. And little or no added sulphur — a touch at bottling is fine and often sensible; drowning the wine in it is not.'),
