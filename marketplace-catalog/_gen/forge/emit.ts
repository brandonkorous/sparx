// Forge generator — the MULTI-FILE payload emitter (docs/85). The shipped payload is a
...
// imports). Why multi-file: a human editing the shipped artifact opens a scoped file
// (e.g. parts/pages/home/01-hero.ts), never one multi-thousand-line wall — and a re-run
// of the generator overwrites it cleanly.
//
...
export async function emitBundle(bundleDir: string, manifest: ManifestLike): Promise<void> {
  ...
  console.log(`gen-forge: wrote multi-file payload — blueprint.ts entry + ${imports.length} parts`);
}
