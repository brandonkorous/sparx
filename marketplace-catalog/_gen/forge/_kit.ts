// Forge generator — shared authoring KIT. Every module composes trees through the
...
//
// Forge is a HYBRID author (see blueprints/forge/README.md), the same posture Mosaic
// proved out:
...
//                       INTERIOR UI (the case-study showcase, gradient project thumbs,
//                       numbered service rows, the awards marquee, quote figures, the
//                       4-up stat band) — the same helpers the platform component
//                       catalog is authored with.
...
let nid = 0;
/** The next globally-stable `fg-<n>` id (increments per call). */
const nextId = (): string => `fg-${(nid += 1)}`;
 ...
 *  awards marquee. The storefront runtime hydrates it and wires the `part()` parts. */
export const behave = (
  n: BuilderNode,
