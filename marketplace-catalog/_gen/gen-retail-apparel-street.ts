...
// accessories (every garment sized XS–XXL and offered in a handful of colors), categories +
...
//   pnpm --filter @wizeworks/api-rest exec tsx "$PWD/marketplace-catalog/_gen/gen-retail-apparel-street.ts"
...
// Loud streetwear: a cool CONCRETE ground (not white — the color of a warehouse floor), a
 ...
 *  brand color itself, so the red does the shouting; text is never set on the photo. */
function hero(): Node {
 ...
 *  uppercase statement in white on near-black, one red CTA. No photo; the color block IS the
 *  device. */
 ...
 *  low-stock, description, size/color add-to-cart, a static "fit & fabric" note, and policy
...
const SEARCH: Node[] = [
    pageMasthead('Search', 'After a specific print, a size or a color? Search the whole shop and the journal below.'),
];
const CART: Node[] = [
 ...
 *  colors that share a prefix (Concrete / Cardinal) never collide in a SKU. */
...
/** A garment — sized XS–XXL and offered in a set of colors; the Size × Color grid of
...
/** An accessory — no size, offered in a set of colors only. */
...
const PRODUCTS: Product[] = [
    ...
    accessory({
        ...
        title: 'Six-Panel Cap',
        description:
            'A structured six-panel cap in heavy cotton twill with an embroidered bird mark, a curved brim broken in just enough, and a metal buckle strap. The finisher on any fit, in colors that go with all of it.',
        price: 38,
        sku: 'CARD-CAP',
