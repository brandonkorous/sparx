...
// complete, working shop the moment it installs — a real catalogue (colour-pop notebooks in
// four colours, a gel-pen set, sticker packs, an enamel pin, a desk calendar, greeting cards,
...
const PHOTO: Record<string, string> = {
  ...
  "prod-colour-notebook": "https://images.unsplash.com/photo-1523742415007-403d0717913a?ixid=M3wxMjA3fDB8MXxzZWFyY2h8Mnx8Y29sb3VyLXBvcCUyMG5vdGVib29rJTIwYnJpZ2h0JTIwY292ZXJ8ZW58MHwwfHx8MTc4NjQwNjM0MXww&ixlib=rb-4.1.0&w=1600&q=80&fit=crop",
...
const ASSETS: Asset[] = [
  { id: 'pop-hero', url: src('pop-hero'), alt: 'A bright desk covered in colourful notebooks, pens and stickers' },
  ...
  { id: 'pop-band-happy', url: src('pop-band-happy'), alt: 'Hands doodling in a colourful notebook at a sunny desk' },
  { id: 'pop-band-gift', url: src('pop-band-gift'), alt: 'A wrapped stationery gift bundle tied with a bright ribbon' },
  { id: 'prod-colour-notebook', url: src('prod-colour-notebook'), alt: 'A colour-pop notebook with a bright cover' },
  { id: 'prod-pocket-trio', url: src('prod-pocket-trio'), alt: 'A trio of small pocket notebooks in bright colours' },
  { id: 'prod-gel-pens', url: src('prod-gel-pens'), alt: 'A set of twelve gel pens in a rainbow of colours' },
  ...
  { id: 'prod-desk-calendar', url: src('prod-desk-calendar'), alt: 'A standing desk calendar with colourful months' },
  { id: 'prod-greeting-cards', url: src('prod-greeting-cards'), alt: 'A boxed set of bright greeting cards with envelopes' },
  { id: 'prod-washi', url: src('prod-washi'), alt: 'A set of patterned washi tape rolls in bright colours' },
  ...
  { id: 'post-desk-setup', url: src('post-desk-setup'), alt: 'A colourful, well-organised happy desk from above' },
  { id: 'post-gift-ideas', url: src('post-gift-ideas'), alt: 'A flat-lay of small stationery gifts wrapped in bright paper' },
  { id: 'post-creativity', url: src('post-creativity'), alt: 'An open notebook full of doodles, lists and colour' },
];
...
/** A loud COLOUR-BLOCK hero — a solid bubblegum panel carrying the rounded display headline
      ...
      el('div', 'mx-auto grid w-full max-w-6xl items-center gap-8 @3xl:grid-cols-2 @3xl:gap-12', {
        children: [
          el('div', 'flex flex-col gap-6 rounded-box bg-primary p-8 @3xl:p-12', {
            children: [
              ...
              el('p', 'text-lg font-semibold leading-relaxed text-primary-content @3xl:text-xl', {
                text: 'Pencil Club is a bright little shop of notebooks, pens, stickers and gifts — the colourful stuff that makes writing a list feel like a good time. Loud on purpose, useful on purpose.',
              }),
          ...
          el('img', 'aspect-square w-full rounded-box bg-base-200 object-cover transition hover:-rotate-2 hover:scale-105', {
            attrs: { src: assetUrl('pop-hero'), alt: 'A bright desk covered in colourful stationery', loading: 'lazy' },
          }),
        ],
      }),
    ],
...
/** One category tile — a photo in a thick coloured frame with a bold label beneath. The whole
      ...
      el('div', 'mx-auto flex w-full max-w-6xl flex-col gap-8', {
        children: [
          ...
          el('div', 'grid grid-cols-2 gap-4 @3xl:grid-cols-4 @3xl:gap-6', {
            children: [
              categoryTile({ assetId: 'pop-tile-notebooks', label: 'Notebooks', href: '/shop', frame: 'border-primary', alt: 'A fan of colourful notebooks' }),
...
/** A row of three loud colour-block promise cards — the shop's cheerful reasons-to-buy, each
 *  a solid role fill with its own `-content` ink. Colour IS the design here: three cards, three
 *  hues, said once. */
      ...
      el('div', 'mx-auto flex w-full max-w-6xl flex-col gap-8', {
        children: [
          ...
          el('div', 'grid grid-cols-1 gap-4 @3xl:grid-cols-3 @3xl:gap-6', {
            children: [
              card('bg-primary', 'text-primary-content', 'Colour on tap', 'Everything here comes in a colour worth reaching for. Pick a mood, not a boring black one.'),
...
/** A full-bleed colour band — a solid role fill carrying a big rounded headline, a lead and a
 *  link, centred. The loud, cheerful counterpart to a photo band; colour does the work. */
function colourBand(o: { heading: string; lead: string; cta: string; href: string; bg: string; ink: string }): Node {
...
const HOME: Node[] = [
  ...
  productsBlock({ source: 'commerce.category.notebooks', layout: 'carousel', heading: 'Notebooks in every colour' }),
  colourBand({
...
const SHOP: Node[] = [
  pageMasthead(
    ...
    'Every bright thing we make — notebooks in four colours, gel-pen sets, sticker packs and pins, a desk calendar, cards, washi and gift bundles. Filter by category or sort however you like; everything ships within two days, wrapped with a smile.'
  ),
];
const COLLECTIONS: Node[] = [
  pageMasthead(
    'Collections',
    'The shop grouped the way people actually shop — the fresh drops, the desk basics, the colour crew, and the gifts that always land. Find your thing, or find one for a friend.'
  ),
];
const SEARCH: Node[] = [
  pageMasthead('Search Pencil Club', 'After a colour, a sticker theme or a gift idea? Search the whole shop and the journal below.'),
];
const CART: Node[] = [
...
/** A notebook, offered in four bright colours — the one thing a Pencil Club notebook is all
 *  about varying on. */
const colourNotebook = (opts: {
  handle: string;
  title: string;
  description: string;
  price: number;
  ...
  options: [COLOUR],
  variants: [
    { sku: `${opts.sku}-SUN`, priceCents: money(opts.price), isDefault: true, inventoryPolicy: 'continue', optionValues: { Colour: 'Sunshine' } },
    { sku: `${opts.sku}-BUB`, priceCents: money(opts.price), inventoryPolicy: 'continue', optionValues: { Colour: 'Bubblegum' } },
    { sku: `${opts.sku}-MIN`, priceCents: money(opts.price), inventoryPolicy: 'continue', optionValues: { Colour: 'Mint' } },
    { sku: `${opts.sku}-GRP`, priceCents: money(opts.price), inventoryPolicy: 'continue', optionValues: { Colour: 'Grape' } },
  ],
...
const PRODUCTS: Product[] = [
  colourNotebook({
    handle: 'colour-pop-notebook',
    title: 'Colour-Pop Notebook',
    description:
      'Our hero notebook — a chunky A5 with a bright, wipe-clean cover and 160 pages of smooth 100gsm dot-grid stock that takes a gel pen or a fountain pen without bleeding through. Lies flat, rounded corners that don’t get bashed, and an elastic in a matching colour. The one that makes you want to fill it.',
    price: 16,
    sku: 'PC-NB-A5',
    collections: ['new-drops', 'best-sellers', 'colour-crew'],
    tags: ['notebook', 'dot-grid', 'a5', 'colourful'],
    asset: 'prod-colour-notebook',
    alt: 'A colour-pop A5 notebook with a bright cover',
    seoTitle: 'Colour-Pop Notebook — bright A5 dot-grid notebook | Pencil Club',
  ...
  colourNotebook({
    ...
    description:
      'Three slim pocket notebooks that live in a bag, a pocket or a glovebox — the place a random thought actually gets caught. Forty-eight pages each of the same smooth stock as the big one, in three colours that make them easy to tell apart. Small, cheap enough to actually use, and far too cute to leave in the shop.',
    price: 12,
    sku: 'PC-NB-PKT',
    collections: ['best-sellers', 'colour-crew', 'desk-basics'],
    tags: ['notebook', 'pocket', 'set', 'colourful'],
    asset: 'prod-pocket-trio',
    alt: 'A trio of small pocket notebooks in bright colours',
    seoTitle: 'Pocket Notebook Trio — bright pocket notebooks | Pencil Club',
    seoDescription: 'A set of three slim pocket notebooks in bright colours, 48 pages each. Sunshine, Bubblegum, Mint or Grape.',
  }),
  {
    handle: 'gel-pen-set',
    title: 'Gel Pen Set of Twelve',
    description:
      'A dozen gel pens in a rainbow that actually looks like a rainbow — smooth 0.7mm rollers that lay down bold, glossy colour and dry fast enough that lefties don’t smudge. No skipping, no scratchy start, no sad muddy shades. The set that turns a to-do list into a small art project.',
    status: 'active',
    productType: 'Pens',
    vendor: 'Pencil Club',
    tags: ['pens', 'gel-pen', 'set', 'colourful'],
    categoryHandles: ['pens-stickers'],
    collectionHandles: ['new-drops', 'best-sellers', 'colour-crew'],
    seoTitle: 'Gel Pen Set of Twelve — rainbow gel pens | Pencil Club',
    seoDescription: 'A set of twelve smooth 0.7mm gel pens in bold, fast-drying rainbow colours. No skipping, no smudging.',
  ...
  },
  {
    ...
    categoryHandles: ['pens-stickers'],
    collectionHandles: ['new-drops', 'colour-crew', 'gift-it'],
  ...
  },
  {
    ...
    title: 'Enamel Pin',
    description:
      'A shiny hard-enamel pin for a jacket, a bag or a lanyard — bright colour, a proper rubber clutch that won’t bail on you, and a shape that makes people smile in the queue. Tiny, collectible, and the easiest little “I saw this and thought of you” there is.',
    status: 'active',
    productType: 'Pin',
    vendor: 'Pencil Club',
  ...
  },
  {
    ...
    description:
      'A cheerful standing calendar that turns the whole year into a bit of colour on your desk — one bright, illustrated spread a month, a fold-out easel back that actually stands up, and big date boxes with room to scribble. Undated? No — properly dated, so you can just flip and go.',
    status: 'active',
    productType: 'Calendar',
    vendor: 'Pencil Club',
    tags: ['calendar', 'desk', 'colourful'],
    ...
    images: [{ assetId: 'prod-desk-calendar', isPrimary: true, alt: 'A standing desk calendar with colourful months' }],
  },
  {
    ...
    vendor: 'Pencil Club',
    tags: ['sticky-notes', 'desk', 'colourful'],
    categoryHandles: ['desk'],
    collectionHandles: ['best-sellers', 'desk-basics', 'colour-crew'],
  ...
  },
  {
    ...
    description:
      'Five rolls of low-tack Japanese paper tape for the small joys of a page — flagging a spread, taping in a ticket, or just brightening a boring corner. Writes over cleanly, tears by hand, peels off without a fight. Sold as a set that genuinely goes together, in colours that play nicely with everything else here.',
    status: 'active',
    productType: 'Accessory',
    vendor: 'Pencil Club',
    tags: ['washi', 'tape', 'set', 'colourful'],
    categoryHandles: ['cards-gifts'],
    collectionHandles: ['colour-crew', 'gift-it'],
    ...
    options: [
      { name: 'Colour', displayType: 'swatch', values: [{ value: 'Brights' }, { value: 'Pastels' }, { value: 'Mono' }] },
    ],
    variants: [
      { sku: 'PC-WASHI-BRT', priceCents: money(11), isDefault: true, inventoryPolicy: 'continue', optionValues: { Colour: 'Brights' } },
      { sku: 'PC-WASHI-PAS', priceCents: money(11), inventoryPolicy: 'continue', optionValues: { Colour: 'Pastels' } },
      { sku: 'PC-WASHI-MON', priceCents: money(11), inventoryPolicy: 'continue', optionValues: { Colour: 'Mono' } },
    ],
    images: [{ assetId: 'prod-washi', isPrimary: true, alt: 'A set of patterned washi tape rolls in bright colours' }],
  },
  {
    handle: 'happy-desk-bundle',
    title: 'The Happy Desk Bundle',
    description:
      'The whole cheer-up in one box — a Colour-Pop Notebook, the twelve gel pens, a sticker pack and a rainbow sticky-note cube, wrapped and ready to give. It’s what we reach for when someone “just needs a little something”: everything to make a desk fun, and nothing to figure out. Add a note at checkout and we’ll write it in.',
    status: 'active',
    productType: 'Gift set',
    vendor: 'Pencil Club',
    tags: ['gift', 'bundle', 'set', 'colourful'],
    categoryHandles: ['cards-gifts'],
    ...
    seoDescription: 'A boxed gift set: a Colour-Pop Notebook, twelve gel pens, a sticker pack and a rainbow sticky-note cube, wrapped and ready to give.',
...
const COMMERCE = {
  categories: [
    { handle: 'notebooks', name: 'Notebooks', description: 'Bright notebooks in every colour.', featured: true },
  ...
  collections: [
    {
      ...
      type: 'manual',
      featured: true,
      productHandles: ['colour-pop-notebook', 'gel-pen-set', 'sticker-pack', 'enamel-pin', 'desk-calendar'],
    },
    {
      handle: 'best-sellers',
      name: 'Best sellers',
      ...
      type: 'manual',
      featured: true,
      productHandles: ['colour-pop-notebook', 'pocket-notebook-trio', 'gel-pen-set', 'sticky-note-cube', 'happy-desk-bundle'],
    },
    {
      handle: 'colour-crew',
      name: 'The colour crew',
      description: 'The brightest stuff we make.',
      type: 'manual',
      featured: false,
      productHandles: ['colour-pop-notebook', 'pocket-notebook-trio', 'gel-pen-set', 'sticker-pack', 'sticky-note-cube', 'washi-tape-set'],
    },
    {
      handle: 'desk-basics',
      name: 'Desk basics',
    ...
    body: {
      ...
      body: {
        ...
        content: [
          ...
          para('Start by sweeping everything off the main surface and only putting back what earns its spot — the notebook you’re using, the pens you actually reach for, and one thing that just makes you smile: a pin on a jar, a sticker on the laptop, a rainbow cube of sticky notes. A clear surface plus one hit of colour beats a tidy-but-grey desk every time.'),
          h2('Give jobs a home and a colour'),
          para('The trick that makes it stick is colour-coding by feel, not by system. Today’s list gets a bright sticky note where you can’t miss it; the notebook gets a colour you like opening; the calendar sits where you’ll actually glance at it. When a task has a colour and a place, you stop losing it — and the desk starts working with you instead of against you.'),
        ],
      },
    },
  },
    ...
    body: {
      ...
      body: {
        ...
        content: [
          ...
          para('Give them a reason to slow down for five minutes: a Colour-Pop notebook and the gel-pen set, so the endless to-do list becomes something almost fun to write. It says “I see how much you’ve got on” without being a self-help book, and it’s the gift they’ll actually use every single day.'),
          h2('For the person who has everything'),
          para('Go small, go colourful, go consumable. An enamel pin they’d never pick, a sticker pack for their laptop, a box of bright cards for the notes they mean to send. Little things that get used up and don’t clutter a shelf — and if you’re not sure, the Happy Desk Bundle is the whole grin in one box. Add a note at checkout and we’ll wrap it and write it in.'),
        ],
      },
    },
  },
...
const SPEC: TemplateSiteSpec = {
  ...
  seo: {
    home: {
      ...
      description:
        'Pencil Club is a bright little shop of colourful notebooks, gel pens, stickers, pins and gifts — the fun, useful desk stuff that makes writing a list feel like a treat. Wrapped with a smile, posted fast.',
    },
    about: {
