...
//   pnpm --filter @sparx/api-rest exec tsx "$PWD/marketplace-catalog/_gen/gen-b2b-apparel-blanks.ts"
...
// clean catalogue feel, one confident cobalt accent as the single pop of colour, under a
...
const ASSETS: Asset[] = [
  { id: 'blanks-hero', url: src('blanks-hero'), alt: 'Folded stacks of blank tees in graded colours on warehouse shelving' },
  ...
  { id: 'tile-headwear', url: src('tile-headwear'), alt: 'A row of blank structured caps in several colours' },
...
const SHOP: Node[] = [
  pageMasthead(
    'Shop blanks',
    'The full stock — tees, fleece, headwear and bags, all sold by the case and priced per unit. Filter by category or colour, or sort by price; every style is blank and ready to screenprint, embroider or DTG.'
  ),
];
...
const SEARCH: Node[] = [
  pageMasthead('Search Blankstock', 'Looking for a style, a weight, a colour or a size run? Search the whole catalogue and the print guides below.'),
];
const CART: Node[] = [
    ...
    children: [
      el('div', 'mx-auto flex w-full max-w-xl flex-col items-center gap-5', {
        children: [
          ...
          el('p', 'text-lg leading-relaxed text-base-content', {
            text: 'Tell us about your shop — what you print, roughly how much you run, and what you’re looking for. We’ll set you up with trade pricing, net-30 terms once you’re approved, and a rep who knows blanks. No membership fee, no minimum to start.',
          }),
          el('a', 'btn btn-primary btn-lg', { attrs: { href: 'mailto:trade@blankstock.example' }, text: 'Email the trade desk' }),
        ],
      }),
    ],
  }),
...
// Named colour codes for SKU building — a stable code per colour so a reorder resolves the
 ...
 *  per colour/size), per-unit priced with a 2XL upcharge, the first the default. */
const garment = (opts: {
  handle: string;
  title: string;
  description: string;
...
const PRODUCTS: Product[] = [
  ...
  accessory({
    ...
    title: 'Cotton Tote',
    description:
      'A 6 oz cotton canvas tote with reinforced handles and a flat, boxy front — a large, forgiving print area for a one-colour logo or a full-bleed job. The add-on blank that lifts an order value and moves at events.',
    price: 4,
    sku: 'BLK-TOTE',
    productType: 'Bag',
    ...
    body: {
      ...
      body: {
        ...
        content: [
          ...
          para('Plastisol and water-based inks sit best on a tight, ring-spun face with enough weight to hide the platen behind it. A 5.5–6.0 oz tee is the safe default for multi-colour and heavy coverage; lighter blanks work for one- and two-colour jobs but show the platen and can grin through on a stretch. Side-seamed bodies lie flatter on the pallet, which keeps registration honest across a run.'),
    ...
    body: {
      ...
      body: {
        ...
        content: [
          ...
          para('If a design sells and you reorder, you want the second batch to match the first on the shelf. Ordering from deep, steady stock in the same colourway keeps a black looking like the same black three months later — the difference between a clean reorder and a customer asking why the new shirts look off.'),
        ],
      },
    },
  },
