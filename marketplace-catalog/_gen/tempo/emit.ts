// Tempo generator — the MULTI-FILE payload emitter (docs/85). The shipped payload is a
// THIN entry (blueprint.ts) that imports each big section from ./parts/* and
// default-exports the assembled manifest. The ingest dynamic-imports the entry; ESM
// resolves the relative parts, so the payload stays self-contained (pure data, no @sparx
// imports). Why multi-file: a human editing the shipped artifact opens a scoped file (e.g.
// parts/pages/home/01-hero.ts), never one multi-thousand-line wall — and a re-run of the
// generator overwrites it cleanly.
//
// All node ids are already minted (the manifest passed in is fully built); this module
// only serializes — so it is order-independent and depth-agnostic.
...
// Just the slice of the manifest the emitter walks — everything else rides through on the
...
const partHeader = (what: string): string =>
  `// ${BRAND} — ${what} (GENERATED payload part, docs/85). Pure data, no imports.\n` +
...
export async function emitBundle(bundleDir: string, manifest: ManifestLike): Promise<void> {
  // `writeModule` writes one ./parts/<rel> data module: optional import lines at the top
  // (relative to THIS module's own folder), then `export default` the value with every
  // `__REF__<var>` sentinel unquoted into the bare import identifier.
  ...
  // `topPart` writes a self-contained data/tree module at parts/<file>.ts and records the
  // entry's import. Returns the entry-side `__REF__` sentinel.
  ...
  // each direct child section becomes its OWN file under parts/<dir>/<base>/NN-name.ts and
  // the tree module re-imports them — so even the home page is a FOLDER of small section
  // files, never one wall. The ingest resolves the import chain transitively.
  ...
  const entry: Record<string, unknown> = {
    ...
    layout: {
      ...manifest.layout,
      tree: await topPart('layout', 'layoutTree', 'site layout (utility · header · footer)', manifest.layout.tree),
    },
  };
  if (manifest.content !== undefined)
    entry.content = await topPart('content', 'content', 'CMS content (news posts)', manifest.content);
  ...
  const entryFile =
    `// ${BRAND} — blueprint payload ENTRY (GENERATED, docs/85). THIN: it imports each\n` +
  ...
  console.log(`gen-tempo: wrote multi-file payload — blueprint.ts entry + ${imports.length} parts`);
}
