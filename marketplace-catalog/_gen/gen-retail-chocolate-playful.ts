...
//
// The joyful, colorful counterpart to the serious bean-to-bar chocolatier (gen-retail-chocolate,
// "Cacao & Co."). A complete, working shop the moment it installs — a real catalogue of colorful
...
// characterful display, big colorful confection photography and playful hovers (tiles that scale
...
//   pnpm --filter @wizeworks/api-rest exec tsx "$PWD/marketplace-catalog/_gen/gen-retail-chocolate-playful.ts"
...
const ASSETS: Asset[] = [
    { id: 'choc-hero', url: src('choc-hero'), alt: 'A colorful spread of glossy filled bonbons and wrapped treats on a bright table' },
    { id: 'tile-boxes', url: src('tile-boxes'), alt: 'An open box of jewel-colored bonbons' },
    ...
    { id: 'band-make', url: src('band-make'), alt: 'A chocolatier piping colorful shells into bonbon moulds' },
    { id: 'band-gift', url: src('band-gift'), alt: 'A wrapped treat box left on a doorstep with a balloon' },
    { id: 'prod-rainbow', url: src('prod-rainbow'), alt: 'A box of rainbow-colored filled bonbons' },
    ...
    { id: 'prod-pretzels', url: src('prod-pretzels'), alt: 'Chocolate-dipped pretzels with colorful drizzle' },
    ...
    { id: 'post-makers', url: src('post-makers'), alt: 'Chocolatiers decorating trays of colorful bonbons' },
];
...
/** A full-bleed hero — one bright, colorful photograph, a rounded headline and a lead in a
    ...
    return el('section', 'relative @container overflow-hidden bg-base-200', {
        children: [
            el('img', 'absolute inset-0 h-full w-full object-cover', {
                attrs: { src: assetUrl('choc-hero'), alt: 'A colorful spread of bonbons and treats', loading: 'lazy' },
            }),
            el(
                'div',
            ...
            el('div', 'mx-auto flex w-full max-w-6xl flex-col gap-8', {
                children: [
                    ...
                    el('div', 'grid grid-cols-2 gap-4 @3xl:grid-cols-4 @3xl:gap-6', {
                        children: [
                            categoryTile({ assetId: 'tile-boxes', label: 'Boxes', href: '/shop', alt: 'An open box of colorful bonbons' }),
...
const HOME: Node[] = [
    ...
    editorialBand({
        ...
        alt: 'A chocolatier piping colorful bonbon shells',
    }),
...
const CONTACT: Node[] = [
    ...
    contactSection({
        heading: 'Say hello',
        intro: 'A question about a flavour, a big order for a party or a wedding, or want a custom box with your colors on it? Tell us what you’re dreaming up and a real person at the kitchen will get right back to you.',
...
// A colorful bonbon box — the shop's hero product shape. Box size only; the flavour IS the box.
const box = (opts: {
    handle: string;
    title: string;
    description: string;
...
const COMMERCE = {
    categories: [
        { handle: 'boxes', name: 'Boxes', description: 'Colorful bonbon and assortment boxes.', featured: true },
