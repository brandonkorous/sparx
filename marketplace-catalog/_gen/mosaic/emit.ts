// Mosaic generator — the MULTI-FILE payload emitter (docs/85). The shipped payload is
// a THIN entry (blueprint.ts) that imports each big section from ./parts/* and
// default-exports the assembled manifest. The ingest dynamic-imports the entry; ESM
// resolves the relative parts, so the payload stays self-contained (pure data, no
// @sparx imports). Why multi-file: a human editing the shipped artifact opens a scoped
// file (e.g. parts/pages/home/01-hero.ts), never one multi-thousand-line wall — and a
...
export async function emitBundle(bundleDir: string, manifest: ManifestLike): Promise<void> {
  ...
  if (manifest.content !== undefined)
    entry.content = await topPart('content', 'content', 'CMS content (customer stories)', manifest.content);
  const pages = [];
  ...
  console.log(`gen-mosaic: wrote multi-file payload — blueprint.ts entry + ${imports.length} parts`);
}
