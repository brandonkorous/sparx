...
//   pnpm --filter @sparx/api-rest exec tsx "$PWD/marketplace-catalog/_gen/gen-retail-wine-modern.ts"
...
/** The wine-club colour band — a solid, punchy berry strip (the one place a full brand fill
...
const SHOP: Node[] = [
  pageMasthead(
    'Shop the shelf',
    'Every bottle we are pouring right now — reds, whites, rosé, bubbles, a monthly wine club and a couple of ready-made cases. Filter by colour or sort however you like; all of it is tasted, approved and ready to drink.'
  ),
];
...
const SEARCH: Node[] = [
  pageMasthead('Search Coupe', 'After a grape, a colour, a price or a bottle for a specific dinner? Search the whole shop and the journal below.'),
];
const CART: Node[] = [
    ...
    children: [
      el('div', 'mx-auto flex w-full max-w-xl flex-col items-center gap-5', {
        children: [
          ...
          el('p', 'text-lg leading-relaxed text-base-content', {
            text: 'After a bottle for a dinner, a case for a party, or a hand picking a gift? Tell us what you are after and a real person from the shop will get right back to you — recommendations always welcome.',
          }),
          el('a', 'btn btn-primary btn-lg', { attrs: { href: 'mailto:hello@coupewine.example' }, text: 'Email the shop' }),
        ],
      }),
    ],
  }),
...
const PRODUCTS: Product[] = [
  ...
  bottle({
    ...
    title: 'Sunday Rosé',
    description:
      'A dry, pale 750ml rosé with none of the sweetness the colour gets blamed for. Wild strawberry, pink grapefruit and a fresh, savoury finish — the house pour from the first warm afternoon to the last. Brunch approved, picnic essential.',
    price: 18,
    sku: 'COUPE-ROSE-01',
  ...
  },
  {
    ...
    description:
      'A ready-made line-up for a table full of friends — six bottles, our pick, at a better price than buying them one by one. Choose a mix of colours to cover every course, all reds for a hearty night, or whites and bubbles for something lighter. We swap the picks every month.',
    status: 'active',
    ...
    seoDescription: 'A curated six-bottle case at a better price — mixed colours, all reds, or whites and bubbles.',
    options: [
      { name: 'Selection', displayType: 'dropdown', values: [{ value: 'Mixed colours' }, { value: 'All reds' }, { value: 'Whites & bubbles' }] },
    ],
    variants: [
      { sku: 'COUPE-SIX-MIX', priceCents: money(95), isDefault: true, inventoryPolicy: 'continue', optionValues: { Selection: 'Mixed colours' } },
    ...
    body: {
      ...
      body: {
        ...
        content: [
          ...
          para('At its heart, natural wine is wine made with as little added to it as possible — grapes farmed organically, fermented with the wild yeast that arrives on their skins rather than a lab packet, and bottled with little or no added sulphur. No added colour, no added acid, no filtering the life out of it. It is not a legal term, so it lives on a spectrum, but that is the direction of travel.'),
