...
// frames (each hand-finished in a range of tortoise / havana / crystal colourways, with a lens
...
//   pnpm --filter @sparx/api-rest exec tsx "$PWD/marketplace-catalog/_gen/gen-retail-eyewear-heritage.ts"
...
const SHOP: Node[] = [
  pageMasthead(
    'The collection',
    'Every frame the house makes — optical, sun and reading — each hand-finished in its own acetate colourways, with the lens choice built in. Filter by shape or category, or sort however you like; all of it is cut and glazed to order on our bench.'
  ),
];
...
const SEARCH: Node[] = [
  pageMasthead('Search Atelier Optique', 'Looking for a shape, a colourway, or a note from the workshop? Search the whole house and the journal below.'),
];
const CART: Node[] = [
  ...
  el('section', 'bg-base-100 @container px-6 py-20 text-center', {
    children: [
      el('div', 'mx-auto flex w-full max-w-xl flex-col items-center gap-5', {
        children: [
          el('h1', 'text-5xl font-bold tracking-tight text-base-content @2xl:text-6xl', { text: 'Call on the workshop' }),
          el('p', 'text-lg leading-relaxed text-base-content', {
            text: 'Unsure which shape suits you, need a hand reading your prescription, or want a frame repaired or refitted? Write to the bench and one of the makers — not a call centre — will write back to you personally.',
          }),
          el('a', 'btn btn-primary btn-lg', { attrs: { href: 'mailto:bench@atelieroptique.example' }, text: 'Write to the bench' }),
        ],
      }),
    ],
  }),
...
/** A frame product — a Colour swatch (the acetate colourway) × a Lens dropdown (the lens type),
 *  every combination a real, buyable variant. Default is the first colour × first lens. */
const frame = (opts: {
  handle: string;
  title: string;
  description: string;
  ...
  const colours = opts.colours ?? ['Havana', 'Amber Tortoise', 'Smoke Crystal'];
  ...
  return {
    ...
    options: [
      { name: 'Colour', displayType: 'swatch', values: colours.map((v) => ({ value: v })) },
...
const PRODUCTS: Product[] = [
  ...
  frame({
    ...
    colours: ['Dark Havana', 'Ebony', 'Honey Crystal'],
  ...
  frame({
    ...
    colours: ['Rose Tortoise', 'Havana', 'Onyx'],
  ...
  frame({
    ...
    colours: ['Olive Tortoise', 'Amber', 'Crystal'],
  ...
  frame({
    ...
    asset: 'prod-riviera',
    colours: ['Tortoise', 'Bottle Green', 'Ebony'],
  ...
  frame({
    ...
    asset: 'prod-study',
    colours: ['Havana', 'Amber Tortoise', 'Onyx'],
  ...
  frame({
    ...
    asset: 'prod-1932',
    colours: ['Archive Amber', 'Ink Tortoise'],
