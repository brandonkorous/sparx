...
// XS–XL and offered in a handful of colors), categories + collections, a bespoke
...
// accent — the clothes are the color. Shipped as Kestrel.
//
...
//   pnpm --filter @wizeworks/api-rest exec tsx "$PWD/marketplace-catalog/_gen/gen-retail-apparel-minimal.ts"
...
// a color at all — the garments carry the color, the site does not. A Didone display
 ...
 *  description, size/color add-to-cart, a static "considered construction" note, and policy
...
const SEARCH: Node[] = [
    pageMasthead('Find it fast', 'Search Kestrel', 'Looking for a color, a size or a particular cut? Search the whole shop and the journal below.'),
];
const CART: Node[] = [
 ...
 *  colors that share a 3-letter prefix (Chambray / Charcoal) never collide in a SKU. */
...
/** A garment — sized XS–XL and offered in a set of colors; the Size × Color grid of
...
const PRODUCTS: Product[] = [
    ...
    accessory({
        ...
        description:
            'A generously long lambswool scarf, brushed soft and finished with a hand-knotted fringe. Wide enough to wrap twice, light enough to leave on indoors. The finishing piece for the coat, in colors that go with all of it.',
        price: 65,
        sku: 'KES-SCF',
...
const COMMERCE = {
    ...
    collections: [
        ...
        },
        {
            handle: 'new-in',
            name: 'New in',
            description: 'The latest cuts and colors.',
            type: 'manual',
            featured: true,
