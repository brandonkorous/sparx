...
//   pnpm --filter @sparx/api-rest exec tsx "$PWD/marketplace-catalog/_gen/gen-template-ideas-talks.ts"
...
// carry the colour; the chrome stays neutral so they pop (podium's whole strategy).
  ...
  el('section', 'bg-base-100 @container px-6 py-20 text-center', {
    children: [
      el('div', 'mx-auto flex w-full max-w-xl flex-col items-center gap-5', {
        children: [
          el('h1', 'text-4xl font-bold tracking-tight text-base-content @2xl:text-5xl', {
            text: 'Bring us an idea',
          }),
          el('p', 'text-lg leading-relaxed text-base-content', {
            text: 'Have a talk in you, a speaker to nominate, or a question about membership? We read everything, and we answer the ones we can. Tell us what you are thinking about.',
          }),
          el('a', 'btn btn-primary btn-lg', {
            attrs: { href: 'mailto:hello@thecommons.example' },
            text: 'Email The Commons',
          }),
        ],
      }),
    ],
