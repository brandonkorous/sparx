...
const SHOP: Node[] = [
  pageMasthead(
    'Shop eyewear',
    'Every frame we make — optical, sunglasses, blue-light and reading — each in its own colourways with the lens choice built in. Filter by shape or category, or sort however you like; all of it comes with our free home try-on.'
  ),
];
...
const SEARCH: Node[] = [
  pageMasthead('Search Frame & Lens', 'Looking for a shape, a colour, or a lens guide? Search the whole shop and the journal below.'),
];
const CART: Node[] = [
    ...
    children: [
      el('div', 'mx-auto flex w-full max-w-xl flex-col items-center gap-5', {
        children: [
          ...
          el('p', 'text-lg leading-relaxed text-base-content', {
            text: 'Not sure which frame suits you, need help reading your prescription, or want to check on an order? Tell us what you’re after and a real person on our team — not a bot — will get back to you.',
          }),
          el('a', 'btn btn-primary btn-lg', { attrs: { href: 'mailto:hello@framelens.example' }, text: 'Email the team' }),
        ],
      }),
    ],
  }),
...
/** A frame product — a Colour swatch (the frame colourway) × a Lens dropdown (the lens type),
...
const frame = (opts: {
  ...
  const colours = opts.colours ?? ['Tortoise', 'Black', 'Crystal'];
...
const PRODUCTS: Product[] = [
  ...
  frame({
    ...
    asset: 'prod-quill',
    colours: ['Gunmetal', 'Rose gold', 'Black'],
