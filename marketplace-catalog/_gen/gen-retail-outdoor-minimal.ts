...
const SEARCH: Node[] = [
  pageMasthead('Search Tarn', 'Looking for a shell, a size, a colour or a guide? Search the whole shop and the journal below.'),
];
const CART: Node[] = [
      ...
      el('div', 'mx-auto flex w-full max-w-6xl flex-col gap-4', {
        children: [
          ...
          el('p', 'max-w-2xl text-lg leading-relaxed text-base-content', {
            text: 'Free carbon-neutral shipping over $75, and easy 30-day returns on everything. Between sizes or not sure a colour’s right? Tell us before you check out and we’ll help you get it right the first time.',
          }),
        ],
      }),
    ],
  ...
  el('section', 'bg-base-100 @container px-6 py-20 text-center', {
    children: [
      el('div', 'mx-auto flex w-full max-w-xl flex-col items-center gap-5', {
        children: [
          el('h1', 'text-5xl font-semibold tracking-tight text-base-content @2xl:text-6xl', { text: 'Get in touch' }),
          el('p', 'text-lg leading-relaxed text-base-content', {
            text: 'Questions about fit, a colour, a repair or an order? Tell us what you’re after and a real person who wears the pieces will get back to you — usually within a day.',
          }),
          el('a', 'btn btn-primary btn-lg', { attrs: { href: 'mailto:studio@tarn.example' }, text: 'Email the studio' }),
        ],
      }),
    ],
  }),
 ...
 *  colours that share a 3-letter prefix (Slate / Storm) never collide in a SKU. */
...
/** A colour-only good — no size, offered in a set of colours (packs, totes, caps, socks). */
