...
//
// ── Why this lives in @sparx/commerce ────────────────────────────────────────
//
// It is the only package that can see both sides. `@sparx/inventory` owns the
// packing record and must not depend on `@sparx/crm` — the dependency rule points
// consumers AT inventory, never the reverse — and `@sparx/crm` owns the order and
...
import type { OrderFulfillment } from '@sparx/db';
