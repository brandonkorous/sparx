...
// instant a list can colour before the promise is missed, rather than a number
...
import type { LocalCalendarParts } from '@sparx/time';
import { formatLocalDate, localCalendarParts, localWallToUtc, nextLocalDay } from '@sparx/time';
 ...
 *
 * These map straight onto colour (RULE #4): breached is danger, warning is
 * amber, met is success, ok is neutral. A support queue where the state is
 * carried by the colour can be read across a room; one where it is carried by
