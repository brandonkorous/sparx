-- Brand content colors (docs/33 §3.1, docs/30 §6): complete the accent `-content`
-- pair and add the secondary identity color (+ its `-content` override) so the
...
-- color and fall secondary back to primary. Additive + nullable, so no backfill;
