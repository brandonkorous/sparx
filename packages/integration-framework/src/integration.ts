// Provider bundles' entry in the shared integration plane (@sparx/integrations).
//
...
//
// `payment` is deliberately unmapped. Payments come from @sparx/payments' gateway
...
function kindFacade(category: IntegrationCategory) {
  switch (category) {
    ...
    default:
      // `dropship` maps to a category, but @sparx/dropship owns that shelf — a bundle
