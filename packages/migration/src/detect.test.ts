...
const MAILCHIMP = [
  'Email Address,First Name,Last Name,OPTIN_TIME,CONFIRM_TIME,MEMBER_RATING,TAGS,FAVOURITE_COLOUR',
...
describe('detect', () => {
  ...
  it('returns nothing for a file it does not know', () => {
    expect(detect({ text: 'colour,size\nred,large' })).toEqual([]);
  });
...
describe('readSource', () => {
  ...
  it('keeps the raw rows and headers for the manual mapper', () => {
    const result = readSource({ text: 'colour,size\nred,large' });
    expect(result.detected).toBeNull();
    expect(result.headers).toEqual(['colour', 'size']);
    expect(result.raw).toEqual([{ colour: 'red', size: 'large' }]);
