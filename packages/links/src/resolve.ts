...
export function buildPath(
  ...
  for (const segment of route.path.split('/').filter(Boolean)) {
    ...
    // A detail address with no record is not an address. The caller gets null
    // and decides — the copy-link control hides itself, the URL tracker falls
    // back to the workbench root.
