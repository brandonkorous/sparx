...
// deliberately withheld from sale. It is NOT the reported `available`
// (`on_hand - allocated`), which stops before the buffer — a buffered level's
// `available` is a number no shopper can ever reach, which is exactly why the
// buffer has to come out before you ask "is this low".
//
 ...
 */
export const SELLABLE_SQL = Prisma.sql`(l.on_hand - l.allocated - l.safety_buffer)`;
...
export function isLowStock(level: {
  ...
  return (
    ...
    level.onHand - level.allocated - level.safetyBuffer <= level.reorderPoint
  );
}
