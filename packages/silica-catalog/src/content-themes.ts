...
// translation") pins an authoritative colour / type / mood direction tuned to look
...
// two-brand-colour institutional chassis (quad — navy primary + crimson accent), an
...
const runway = defineTheme({
  ...
  // the accent role is a muted graphite for whisper-labels only; all colour comes from
...
const console_ = defineTheme({
  ...
  // colour, with ONE electric-violet accent held SCARCE — active nav, the primary
...
const quad = defineTheme({
  ...
  //
  // The set's two-brand-colour institutional chassis: a paper page with a DEEP NAVY
