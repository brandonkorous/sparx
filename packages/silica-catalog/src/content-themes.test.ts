// The bar for the ten bespoke CONTENT-template themes, driven through silica's OWN
// contrast engine — the same `contrastWarnings` / `parseColor` the canvas, theme
// editor and publishing use, so a theme that passes here cannot be flagged there.
//
// The SAME per-theme assertions `themes.test.ts` runs against the trade shelves and
// `template-themes.test.ts` runs against the commerce ten, pointed at `CONTENT_THEMES`:
// a complete light AND dark palette, WCAG AA on every role and on body text over every
// surface, a one-directional surface ramp, a resolvable `-content` for every role,
// shape/type kept out of the dark delta, and the font-token↔self-hosting-record
// pairing. Plus: URL-safe unique names that never shadow a silica preset, one of the
// twenty `SPARX_THEMES`, OR one of the ten `TEMPLATE_THEMES`.
...
import { CONTENT_THEMES, CONTENT_THEME_BY_SLUG } from './content-themes';
...
const bagFor = (theme: (typeof CONTENT_THEMES)[number], mode: 'light' | 'dark') =>
  mode === 'dark' ? { ...theme.tokens, ...theme.dark } : theme.tokens;

describe('the content-template themes', () => {
  it('gives every theme a unique, URL-safe name', () => {
    const names = CONTENT_THEMES.map((t) => t.name);
  ...
  it('never shadows one of silica shipped presets', () => {
    ...
    const collisions = CONTENT_THEMES.map((t) => t.name).filter((n) => shipped.has(n));
    expect(collisions).toEqual([]);
  });

  it('never collides with a SPARX_THEMES shelf name', () => {
    const shelf = new Set(SPARX_THEMES.map((t) => t.name));
    const collisions = CONTENT_THEMES.map((t) => t.name).filter((n) => shelf.has(n));
    expect(collisions).toEqual([]);
  });

  // Both bespoke libraries flatten into the same `[data-theme]` namespace, so a content
  // theme must not collide with one of the commerce ten either.
  it('never collides with a TEMPLATE_THEMES commerce name', () => {
    const commerce = new Set(TEMPLATE_THEMES.map((t) => t.name));
    const collisions = CONTENT_THEMES.map((t) => t.name).filter((n) => commerce.has(n));
    expect(collisions).toEqual([]);
  });

  it('maps every content blueprint slug to one of the ten themes', () => {
    const themes = new Set(CONTENT_THEMES);
    const mapped = Object.values(CONTENT_THEME_BY_SLUG);
    expect(mapped).toHaveLength(CONTENT_THEMES.length);
    for (const t of mapped) expect(themes.has(t)).toBe(true);
    expect(new Set(mapped).size).toBe(CONTENT_THEMES.length);
    for (const slug of Object.keys(CONTENT_THEME_BY_SLUG)) {
      expect(slug).toMatch(/^[a-z][a-z0-9-]*$/);
    }
  });
});

describe.each(CONTENT_THEMES)('$name', (theme) => {
