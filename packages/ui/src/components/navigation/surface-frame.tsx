...
function TopStepper({ steps, current, onStepSelect, canSelectStep }: TopStepperProps) {
    return (
        <div className="shrink-0 border-b border-base-300 bg-base-100 px-6 py-4">
                            ...
                            >
                                <span
                                    ...
                                    className={cn(
                                        ...
                                        status === 'upcoming' &&
                                        'text-base-content/60 border-[color-mix(in_oklab,var(--color-base-content)_30%,transparent)] bg-base-100',
                                        status === 'done' &&
...
function FrameToolbar({
    ...
    return (
        <div
            className={cn(
                '@container/toolbar flex h-[52px] shrink-0 items-center gap-3 border-b border-base-300 bg-base-100 px-6 max-[680px]:px-4',
                className
            )}
        >
            {backLabel && onBack && (
...
function TopStepperFrame({
    ...
    return (
        <ActionSlotProvider>
            <div
                className={cn('flex h-full flex-col overflow-hidden bg-base-200', className)}
            >
                ...
                />
                {context && (
                    <p className="text-base-content/60 shrink-0 border-b border-base-300 bg-base-200 px-6 py-2 text-center text-xs">
                        {context}
                    </p>
                )}
...
export function SurfaceSummaryDivider() {
    return <div className="my-1.5 border-t border-base-300" />;
}
...
function FSurfaceFrame({
    ...
    const inner = (
        <div
            className={cn(
                '@container flex h-full min-h-0 flex-col overflow-hidden bg-base-100',
                ...
                variant === 'embedded' &&
                cn(
                    'mx-auto w-full border-x border-base-300',
          ...
          below) — one toolbar, not two. */}
            ...
            single-step form (one step) hides MiniProgress — nothing to show. */}
                <div className="flex min-h-0 flex-col">
                    {steps.length > 1 && (
                        <div className="shrink-0 bg-base-200 px-7 pt-4 pb-1 max-[680px]:px-5">
            ...
            narrow (stacked) rendering lives inside SurfaceStep's scroll body. */}
                {hasSummary && (
                    <aside
                        className={cn(
                            'hidden min-h-0 flex-col overflow-y-auto border-l border-base-300 px-6 py-6 @[720px]:flex',
                            SUMMARY_BG
                        )}
                    >
                        {summary}
    ...
    return (
        <ExternalActionsTargetContext.Provider value={mergeIntoHostChrome ? actionsTarget : undefined}>
            <ActionSlotProvider>
                ...
                >
                    {variant === 'embedded' ? (
                        <div className="h-full overflow-hidden bg-base-200">{inner}</div>
                    ) : (
                        inner
                    )}
                </SurfaceContext.Provider>
...
export function SurfaceFrame({
                    ...
                    <DialogPrimitive.Portal>
                        ...
                        <DialogPrimitive.Content
                            ...
                            className={cn(
                                ...
                                'rounded-2xl border border-base-300 bg-base-100 shadow-md',
    ...
    return (
        <SurfaceContext.Provider value={{ variant: 'page' }}>
            <div
                className={cn(
                    'grid h-screen grid-cols-[340px_1fr] overflow-hidden bg-base-200',
        ...
        <div className="flex items-center gap-2">
          ...
          primary so it can't be mis-clicked — never buried in the summary aside. */}
            ...
            {hasLeftCluster && hasRightCluster && (
                <div aria-hidden className="h-5 w-px shrink-0 bg-base-300" />
            )}
            {hasRightCluster && (
                <div className="flex items-center gap-1">
                    {extra}
    ...
    if (topStepper) {
        ...
        return (
            ...
            the action toolbar below — stay on surface, framing this band. */}
                <div className="min-h-0 flex-1 overflow-y-auto border border-1 border-primary bg-base-200">
              ...
              right of the flush-left header on a wide (no-summary) sheet. */}
                    <div className={cn('w-full px-7 py-6 max-[680px]:px-5', colWidth, !isF && 'mx-auto')}>
                        ...
                        {isF && summary && (
                            <div
                                className={cn(
                                    'mt-6 rounded-xl border border-base-300 px-5 py-4 @[720px]:hidden',
                                    SUMMARY_BG
                                )}
                            >
                                {summary}
