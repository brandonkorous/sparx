...
export const Sidebar = React.forwardRef<HTMLElement, React.HTMLAttributes<HTMLElement>>(
  ({ className, ...props }, ref) => (
    <aside
      ref={ref}
      className={cn(
        'border-base-300 bg-base-100 flex h-full w-56 shrink-0 flex-col gap-1 border-r p-3',
        className
      )}
      {...props}
    />
...
export const SidebarSectionLabel = ({
  ...
  <div
    className={cn('px-2 pb-1 text-xs font-medium tracking-wider uppercase', className)}
    {...props}
  />
);
...
export const SidebarFooter = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div className={cn('border-base-300 mt-auto border-t pt-2', className)} {...props} />
);
    ...
    const iconSpan = icon ? (
      <span
        className={cn(
          ...
          moduleIcon || active ? 'text-module' : undefined
        )}
      >
        {icon}
      </span>
