  ...
  ({ className, color = 'module', style, ...props }, ref) => {
    ...
    return (
      <SliderPrimitive.Root
        ...
        {...props}
      >
        <SliderPrimitive.Track className="relative h-1.5 w-full grow overflow-hidden rounded-full bg-[var(--color-base-300)]">
        ...
        {value.map((_, i) => (
          <SliderPrimitive.Thumb
            key={i}
            className={cn(
              'block h-4 w-4 rounded-full border-2 border-[var(--sx-sel)] bg-[var(--color-base-100)]',
