  ...
  ({ className, color = 'module', style, ...props }, ref) => {
    ...
    return (
      <SliderPrimitive.Root
        ...
        {...props}
      >
        <SliderPrimitive.Track className="bg-base-300 relative h-1.5 w-full grow overflow-hidden rounded-full">
        ...
        {value.map((_, i) => (
          <SliderPrimitive.Thumb
            key={i}
            className={cn(
              'bg-base-100 block h-4 w-4 rounded-full border-2 border-[var(--sx-sel)]',
