...
- **`indeterminate` is a DOM property with no HTML attribute**, so a tri-state checkbox needs a callback ref (`SelectionList`'s select-all header and workbench's reorder list both do this). silicaui 0.43 styles `:checked` but not `:indeterminate`, so such a checkbox paints as UNCHECKED. One rule closes it, in **`@sparx/brand/silica-gaps.css`** — read that file's header before adding anything to it. It lives in `@sparx/brand`, not here, because **workbench imports no `@sparx/ui` CSS at all** and workbench is where it bites; a rule added to `tokens.css` would silently miss the app that needs it.
