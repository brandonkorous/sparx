// Resolve the web PROPERTY (site) an MCP Builder call targets.
//
// The MCP transport authenticates a tenant + user but carries no property; the
// Builder page services are per-property (docs/49). This mirrors api-rest's
// lib/property.ts so MCP and REST resolve a site identically: an explicit
// `propertyId` wins WHEN it names one of the tenant's OWN properties (RLS scopes
// the lookup, so a forged id fails closed to the primary), otherwise the tenant's
// primary site. Single-site tenants never pass one and behave exactly as before.
//
...
async function resolvePrimarySite(tenantId: string): Promise<ResolvedSite> {
  ...
  if (!row) throw new BuilderNotFoundError('Property', `primary for tenant ${tenantId}`);
  return row;
}

/** Lift an MCP ServiceContext to the PropertyContext the Builder page services
 *  need. `requested` (the optional `propertyId` tool arg) is honored only when it
 *  resolves to one of the tenant's own sites; otherwise the primary. The returned
 *  context also carries the resolved site (`.site`) for the write echo. */
