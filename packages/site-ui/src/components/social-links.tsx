...
// when there are no items. Known platforms render a brand ICON in a circular
// button (theme-agnostic — the circle tints from `currentColor`); an unknown
// platform falls back to its capitalized name as text so nothing is lost.
      ...
      {items.map((item, i) => {
        ...
        return (
          <a
            ...
            className={cx('st-social__link', icon && 'st-social__link--icon')}
          >
            {icon ?? item.platform}
          </a>
        );
