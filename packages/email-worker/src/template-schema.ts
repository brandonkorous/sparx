...
// `team-invitation` and `partner-welcome` were registered in `@sparx/email`,
...
// it. `handler.ts` pulls in `@sparx/email` for `renderTemplate`, which drags the
...
//
// Adding a template to `@sparx/email` is not finished until its literal is
...
//
// So: adding a template to `@sparx/email` is not finished until its literal is
