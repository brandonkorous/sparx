...
- **Migrations go through the pipeline, not your laptop** — every managed instance is private-IP/VNet only, so the only thing that can reach one is already inside the cluster. Author against docker Postgres, push to `main`, and the deploy applies it: on Azure, [deploy-azure.yml](.github/workflows/deploy-azure.yml) calls [db-migrate-azure.yml](.github/workflows/db-migrate-azure.yml) before the rollout. RLS/`current_tenant_id()` are hand-edited SQL, not Prisma-generated. Full mechanics + the FORCE-RLS backfill footgun in [packages/db/CLAUDE.md](packages/db/CLAUDE.md).
- **Workflows are named for the PROVIDER; environment is an input.** `deploy-azure.yml` / `deploy-gcp.yml`, `db-migrate-azure.yml` / `db-migrate-gcp.yml` — never `deploy-prod.yml`. Provider dictates the mechanics (auth, registry, how you reach the cluster); `prod` vs `staging` is a `choice` input inside one file. The build workflows are the deliberate exception, named for the REGISTRY that couples them: Artifact Registry is a GCP service, while GHCR is GitHub's and cloud-neutral (`build-images-ghcr.yml` feeds AKS today, but GKE could pull the same images). **The platform runs on Azure**; the GCP set is a live fallback whose triggers are neutered by a `__disabled__` sentinel branch/tag pattern **inside each file** — not by `gh workflow disable`, because GitHub keys that state to the file PATH and a rename silently re-arms it. Re-arm deliberately by editing the sentinel. Map: [.github/workflows/README.md](.github/workflows/README.md).
- **Releases are automated via tags, not PRs.** Every push to `main` runs [auto-tag.yml](.github/workflows/auto-tag.yml), which analyzes conventional-commit messages since the last `v*` tag and pushes a new tag if anything releasable landed. Bumps: `feat:` → minor, `fix:` / `perf:` → patch, `feat!:` or `BREAKING CHANGE:` footer → major; `chore / docs / refactor / ci / build / test / style` don't bump. Force a specific bump via `gh workflow run auto-tag.yml -f bump=major|minor|patch`. **Tagging dispatches nothing** — it marks a release, it does not decide who deploys it. Azure images build from the push itself (path-filtered), and the deploy chains off that build's success. No bot ever opens a PR; code-change PRs remain a human gate as normal.
